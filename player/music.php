...
<div>
	...
	<div>
		<?php
			...
			/*$pic = new _postingpic;
			$res = $pic->read($_POST["postid"]);

			if($res != false)
			{
				$row = mysqli_fetch_assoc($res);
				echo "<div class='divimgborder imagesize'><img alt='Posting Pic' src=' ".($row["spPostingPic"])."' class='originalimg'></div>";
			}
			else
				echo "<div class='divimgborder imagesize'><img alt='Posting Pic' src='../img/no.png' class='originalimg'></div>";*/
		?>
		
		<div><audio id="audio" style="width:50%;" controls autoplay>
		  <source src="data:audio/mp3;base64,<?php echo ($media); ?>">
		</audio></div>
		<div class="hidden entertainment" data-entertain="data:audio/mp3;base64,<?php echo ($media); ?>"></div>
	...
	<div style="background-color:white;margin-top:10px;">
		...
		<div class="videomusic popup" onclick="myFunction()">
			<div class="row">
				<a href="/friends/?profileid=<?php echo $pid;?>"><p class="videoposter" style="cursor:pointer;margin-left:1px;" data-profileid="<?php echo $pid;?>"><span>Posted By :</span><?php echo $poster; ?></p></a>
		...
		<!--Socializing code-->
			...
			{
				$liked=0;
				echo "<span data-likeid='postid". $_POST["postid"] ."' data-toggle='tooltip' data-placement='bottom' title='Like' class='icon-socialise sp-like fa fa-thumbs-o-up' data-postid='" . $_POST["postid"] . "' data-liked='". $liked ."'></span>";
