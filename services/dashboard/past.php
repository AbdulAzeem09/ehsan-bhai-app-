...
$p= new _classified;
$pf     = new _postfield;
$res    = $p->myposted_expire_service($_SESSION['pid']);
//echo $p->ta->sql;
if($res != false){
$pagetion="example";
}
...
<table class="table table-striped table-bordered dashServ display" id="<?php echo $pagetion; ?>">
<thead>
<tr>
<th>Service Name</th>
...
<a href="<?php echo $BaseUrl.'/services/detail.php?postid='.$row['idspPostings'];?>" class="" data-postid="<?php echo $row['idspPostings'];?>"><i title="View" class="fa fa-eye"></i></a>
<a onclick="deletedata('<?php echo $BaseUrl.'/services/delete.php?action=renew&postid='.$row['idspPostings']; ?>')">
<i class="fa fa-check-square-o" aria-hidden="true" title="Activate">



</i></a>
<a href="javascript:void(0)" onclick="permanentDelete(<?php echo $row['idspPostings']; ?>)"><i style="color:red;" class="fa fa-trash" title="Delete"></i></a>
</td>
</tr>
...
echo "<tr style='text-align:center;'><td colspan='4'><h4>No Expired Ads  Found</h4></td></tr>";
...
Swal.fire({
title: 'Are you Sure you want Activate it ?',
icon: 'warning',
