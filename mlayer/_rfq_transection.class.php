...
class _rfq_transection
{
	...
	function __construct() { 
		$this->ta = new _tableadapter("rfq_transection");
 ...
 function readpublicbuyer($buyer_pid){


   	return $this->ta->read("INNER JOIN rfq as d ON t.sell_idspRfq  = d.idspRfq WHERE t.buyer_pid = " . $buyer_pid);
 ...
 function read($pid)
   ...
   function readpublictransection($Buyer_uid){
 ...
 function getpublicrfq_order($qid)
    {
        //return $this->ta->read("WHERE spProfiles_idspProfiles =".$pid);
        return $this->ta->read("WHERE post_idspRfqComment =".$qid);
    }
