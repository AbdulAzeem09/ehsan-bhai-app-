...
class _postingview
{
    ...
    function __construct()
    {
        ...
        $this->tas = new _tableadapter("spsavepost");
        $this->ak = new _tableadapter("sppostings");
        $this->ak->join = "INNER JOIN spsavepost as d ON t.idspPostings = d.spPostings_idspPostings";
        $this->tr = new _tableadapter("sppostings");
        $this->spp = new _tableadapter("spproduct");
        $this->sp1 = new _tableadapter("sprealstate");
    ...
    function profile_real_state($pid)
    {

        return $this->sp1->read("WHERE spProfiles_idspProfiles=$pid");
    }
    ...
    function search_artgallerynew($catid, $txtSearch)
    {
    ...
    {
        return $this->tas->read(" where spProfiles_idspProfiles= " . $pid . " ORDER BY idsavepost DESC");
    }
    // SHOW HOME PRODUCCT 
    ...
    function myExpireProduct($catId, $pid)
    {
        ...
        /*	return $this->ta->read("WHERE saveasdraft = 0 ORDER BY spPostingDate DESC");*/

        return $this->ta->read("WHERE t.idspprofiles = $pid AND t.spPostingVisibility != -3 AND t.spPostingExpDt < CURDATE() ORDER BY spPostingDate DESC");
    }
    // MY TRASH POSTING
    ...
    function singletimelines($postid)
    {
       return $this->ta->read("WHERE t.idspPostings = $postid");
        // echo $this->ta->sql;die('xxxxxx');
    }

    function singletimelines_1($postid)
    {
        return  $this->ak->read("WHERE t.idspPostings = $postid AND d.spPostings_idspPostings = $postid ");
        //echo $this->ak->sql;die("=---------------");
    }
    function singletimelines_2($postid, $pid)
    {
        return      $this->ak->read("WHERE t.idspPostings = $postid AND  d.spProfiles_idspProfiles = $pid  AND d.spPostings_idspPostings = $postid ");
        //echo $this->ak->sql; die("=---------------");
    }
    ...
    {
    	  $postid = $this->sp->escapeString($postid);
        ...
        return $this->sp->read("INNER JOIN sprealstatepics as p ON t.idspPostings = p.spPostings_idspPostings INNER JOIN sppostfield as pf ON t.idspPostings = pf.spPostings_idspPostings WHERE t.spCategories_idspCategory = 3  AND t.spPostingVisibility = -1 AND t.idspPostings = " . $postid . "");
        //echo $this->sp->sql;
    }
    ...
    function renter_id1($postid)
    {
        return $this->sp1->read("WHERE idspPostings=" . $postid);
    }
    ...
    {
        //return $this->ta->read("WHERE t.idspProfiles = " . $profileid . " AND t.spPostingVisibility=0");
        return $this->ta->read("WHERE d.idspProfiles =" . $pid . " AND t.spPostingVisibility = 0", "ORDER BY spPostingDate DESC");
    }
    ...
    function personal_allfilter($gid)
    {
        $gid = $this->spp->escapeString($gid);
        return $this->spp->read("WHERE t.spPostingVisibility = -1 AND t.spPostingExpDt >= CURDATE() AND subcategory = '$gid' ORDER BY spPostingDate DESC");
        //echo $this->spp->sql;
    ...
    function personal($g)
    {
        return $this->spp->read("WHERE t.spPostingVisibility = -1 AND t.spPostingExpDt >= CURDATE() AND subcategory = '$g' ORDER BY spPostingDate DESC");
        //echo $this->spp->sql;
    ...
    function personal_all($start, $limitaa)
    {
        return  $this->spp->read("WHERE t.spPostingVisibility = -1 AND t.spPostingExpDt >= CURDATE() AND sellType = 'Personal' ORDER BY spPostingDate DESC  limit $start, $limitaa");
    }
    ...
    function singleFriendProduct($pid, $catid)
    {
    ...
    {
        $date = date('Y-m-d');
        return $this->ta->read("INNER JOIN sppostfield as f on t.idspPostings = f.spPostings_idspPostings where t.sppostingvisibility=-1 and t.idspcategory = 1 AND f.spPostFieldName = 'Subcategory_' AND f.spPostFieldValue = '$catName' AND t.spPostingExpDt >= $date GROUP BY t.idspPostings ORDER BY spPostingDate DESC");
        //echo $this->ta->sql;
    }
    ...
    {
        $condition = $this->ta->escapeString($cond);
    ...
    {
        $cond = $this->ta->escapeString($cond);
...
*/
    ...
    function getAgentList($category, $pid)
    {
        ...
        return $this->sp->read("where t.sppostingvisibility = -1 and t.spCategories_idspCategory = '" . $category . "'  AND t.spProfiles_idspProfiles= '" . $pid . "' AND t.spPostingPropStatus='Active'   ORDER BY idspPostings DESC");
        //echo $this->sp->sql;
        //die("========");
    }

    //show all agent listing
    function getAgentListBySellerNumber($category, $sellerNumber)
    {
        return $this->sp->read("where t.sppostingvisibility = -1 and t.spCategories_idspCategory = '" . $category . "'  AND t.seller_mnumber= '" . $sellerNumber . "' AND t.spPostingPropStatus='Active'   ORDER BY idspPostings DESC");
    }
    ...
    {
        $pid = $this->spp->escapeString($pid);
        return $this->spp->read("WHERE spProfiles_idspProfiles = " . $pid . " AND spCategories_idspCategory = " . $catid . " ORDER BY idspPostings DESC");
      //  echo $this->spp->sql;
    ...
    {
        return $this->ta->read("INNER JOIN spfavorites AS f ON t.idspPostings = f.spPostings_idspPostings WHERE (spCategories_idspCategory = 16 OR spCategories_idspCategory = 17)  AND (spPostingVisibility = -1 OR spPostingVisibility in(select  spGroup_idspGroup from spProfiles_has_spGroup where spProfiles_idspProfiles in(Select idspProfiles from spProfiles where idspProfiles =" . $pid . "))) AND (t.spProfiles_idspProfiles in (select spProfiles_idspProfilesReceiver from spprofiles_has_spprofiles where spProfiles_idspProfileSender in(select idspProfiles from spProfiles WHERE idspProfiles  =" . $pid . " )) OR t.spProfiles_idspProfiles in (select spProfiles_idspProfileSender from spprofiles_has_spprofiles where spProfiles_idspProfilesReceiver in(select idspProfiles from spProfiles WHERE idspProfiles  =" . $pid . " )) OR d.idspProfiles =" . $pid . ")", "ORDER BY f.id DESC");
    }

    function globaltimelinesFavourite_uid_pid($start, $pid, $uid)
    {
        return $this->ta->read("INNER JOIN spfavorites AS f ON t.idspPostings = f.spPostings_idspPostings WHERE (spCategories_idspCategory = 16 OR spCategories_idspCategory = 17)  AND (spPostingVisibility = -1 OR spPostingVisibility in(select  spGroup_idspGroup from spProfiles_has_spGroup where spProfiles_idspProfiles in(Select idspProfiles from spProfiles where idspProfiles =" . $pid . " AND spUser_idspUser =" . $uid . "))) AND (t.spProfiles_idspProfiles in (select spProfiles_idspProfilesReceiver from spprofiles_has_spprofiles where spProfiles_idspProfileSender in(select idspProfiles from spProfiles WHERE idspProfiles  =" . $pid . " AND spUser_idspUser =" . $uid . " )) AND t.spProfiles_idspProfiles in (select spProfiles_idspProfileSender from spprofiles_has_spprofiles where spProfiles_idspProfilesReceiver in(select idspProfiles from spProfiles WHERE idspProfiles  =" . $pid . " AND spUser_idspUser =" . $uid . " )) OR f.spProfiles_idspProfiles =" . $pid . " AND f.spUserid =" . $uid . ")", "ORDER BY f.id DESC");
     //echo $this->ta->sql;   
    }
    //ORDER BY spPostingDate DESC
    ...
    {
        return $this->ta->read("INNER JOIN sppostfield AS p ON t.idspPostings = p.spPostings_idspPostings WHERE t.idspcategory = $category AND idsppostings in(select sppostings_idsppostings from spfavorites where spprofiles_idspProfiles = $pid) GROUP by idspPostings ORDER BY spPostingDate DESC");
    }
    //real Estate
    ...
    function myReceivedBooking($pid, $type)
    {
        return $this->rb->read("INNER JOIN sprealstate as sp on t.spPosting_idspPosting = sp.idspPostings WHERE sp.spProfiles_idspProfiles = $pid AND t.rs_roomType = '$type'");
        // echo   $this->rb->sql; die("---------------");
    }

    function myReceivedBookingdetails($roombookid)
    {
        return $this->rb->read("WHERE t.spPosting_idspPosting = $roombookid AND t.spStatus = 3");
    }

    function myReceivedBookingRoom($roombookid)
    {
        return $this->rb->read("WHERE t.idspRoomBook = $roombookid");
    }
    // MUSIC MODUELE
    // read post
    ...
    {
        return $this->music_video_user->read(" , music_audio_video AS aaa, spprofiles_has_spprofiles AS bbb WHERE aaa.spUser_idspUser = t.idspUser AND  bbb.spProfiles_idspProfileSender = $logid OR bbb.spProfiles_idspProfilesReceiver = $logid
    ...
    function sameTrainCategory_all($category)
    {
        return  $this->ta->read("INNER JOIN sppostfield as p ON t.idspPostings = p.spPostings_idspPostings WHERE t.spPostingVisibility=-1 AND p.spPostFieldName = 'trainingcategory_'  AND t.spCategories_idspCategory = " . $category, "ORDER BY RAND()");
        //echo $this->ta->sql; 
    }
