...
$result = $p->read_membership_id($_GET['id']);
...
}?><label for="">Edit Membership</label>

<div class="col-md-9 bg_white" style="padding-bottom: 15px; margin-top: 10px; margin-left: 10px;">
 

<form action="update_membership_detail.php" method="post" enctype="multipart/form-data" class="" >  
...
<select id="type" class="form-control" name="type">
    <option  value="0" <?php if($type==0){ echo "selected"; } ?> >Select Option</option>
    <option  value="1" <?php if($type==1){ echo "selected"; }?> >a </option>
    <option  value="2"<?php if($type==2){ echo "selected";} ?> > b</option>
    <option  value="3"  <?php if($type==3){ echo "selected"; } ?> > c</option>
...
<input type="text" class="form-control"  onkeypress="return (event.charCode > 64 && event.charCode < 91) || (event.charCode > 96 && event.charCode < 123)||(event.charCode==32)"  id="shipp_username"  name="fullname" value="<?php echo $membership_name;?>"> 
<span id="shippname_error" style="color:red;"></span>
</div>

<div class="form-group">
<label class="add_shippinglabel" for="shipp_address">Membership Price (<?php echo $currency;?>):<span class="red"></span></label>
<input type="text" class="form-control" id="price" 
name="price" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*?)\..*/g, '$1');" value="<?php echo $membership_price;?>">
...
name="duration"  value="<?php echo $membership_duration;?>">
...
<input type="radio"  name="status" value="1" <?php if($membership_status==1){echo "checked";} ?>>&nbsp;&nbsp;Active<br>
<input type="radio"  name="status" value="0" <?php if($membership_status==0){echo "checked";} ?>>&nbsp;&nbsp;Inactive
...
<div class="form-group">        
<div class="">
<button type="submit" class="btn btn-default Add_adderess">Update Membership</button>  
</div>
</div>
