			...
			if($result2){
				...
				if($result3){
					...
					if($result4){
						...
						$freelancerPid = $row4['idspProfiles'];
						...
						if(isset($BusinessUserId) && $BusinessUserId > 0){
							$sql = "INSERT INTO freelance_account (spUser_idspUser, spProfile_idspProfile, spPosting_idspPostings, fa_debit, fa_credit, fa_current_amount, id_milestone, fa_status, fa_date)VALUES
							('$BusinessUserId', '$BusinessPid', '$projectId', '0', '$milestonePrice','$businessCurrnAmt', '$milestoneId', 'pending', NOW() )";
							$result_sql = mysqli_query($conn, $sql);
							if($result_sql){
								//Amount freelancer k acount main send ki
								if(isset($freelancerUserId) && $freelancerUserId > 0){
									$sql2 = "INSERT INTO freelance_account(spUser_idspUser, spProfile_idspProfile, spPosting_idspPostings, fa_debit, fa_credit, fa_current_amount, id_milestone, fa_status, fa_date)VALUES
									($freelancerUserId, '$freelancerPid', $projectId, $freelanceAmtSend, '0',$frelanceCurrnAmt, $milestoneId, 'pending', NOW() )";
									...
									if($result_sql2){
										...
										$redirctUrl= $BaseUrl."/freelancer/dashboard/active-bid.php";
