...
.dismis_all{background-color: #99068a;
    color: white;
    padding: 10px;
}  
</style>
...
<?php
		...
		if($resnoti!=false){
      ?>
<p class="dismiss text-right"><button id="dismiss-all" class="btn btn-danger btn-border-radius" onclick="loadDocAll(<?php echo $_SESSION['pid']; ?>)" >Dismiss All</button></p>
      <?php
    ...
    <div class="card-body">
       <table>
        <tr>
          ...
          <td style="width:10%">
            <a class="btn btn-danger dismiss-notification btn-border-radius"   onclick="loadDoc(<?php echo $rownoti['id']; ?>)" >Dismiss</a>
          </td>
        </tr>
      </table>
    </div>
...
dismissAll.addEventListener('click', function(e){
  ...
  //setTimeout(function(){	  $('.temp').hide();		        },5000);  
...
const xhttp = new XMLHttpRequest();
  ...
  xhttp.open("GET", "/artandcraft/dashboard/seller-notification.php?to_idup="+to_id);
  xhttp.send();
