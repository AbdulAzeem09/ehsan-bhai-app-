        ...
        include_once("../../header.php");
			  ...
			  if(isset($_GET['orderPid'])){
				...
				if(isset($_GET['producid'])){
			        ...
			        $presp = $p->singletimelines($_GET['producid']);
					$resrpp = mysqli_fetch_array($presp);
					if($resrpp['return_if_applicable']==0){	
					    $action = 1;
				        $o->updateBuyerOrderstatus($_GET['orderPid'], $action);
						
						$opcdata = $o->readBuyerOrdertotalprorow($_GET['orderPid']);
						
						$rowopcdata = mysqli_fetch_array($opcdata);
						
						$price = $rowopcdata['price'];
						$spOrderQty = $rowopcdata['spOrderQty'];
				  ...
				  if(isset($_GET['to_id'])){
					...
					if($datastuts=='Shipped'){
						$message =  '<b>Order Shipped </b>:  Your product of ORDER_'.$orderid.' has been Shipped , <a href="/artandcraft/dashboard/invoice.php?order='.$_GET['r'].'">Click to View</a>';
					}
					if($datastuts=='Out_Of_Delivery'){
						$message =  '<b>Out Of Delivery</b> : Your product of ORDER_'.$orderid.'  is out for delivery, <a href="/artandcraft/dashboard/invoice.php?order='.$_GET['r'].'">Click to View</a>';
					}
					if($datastuts=='Delivered'){
						$message =  '<b>Order Placed </b>: Your product of ORDER_'.$orderid.' has been Delivered, <a href="/artandcraft/dashboard/invoice.php?order='.$_GET['r'].'">Click to View</a>';
					}
					if($datastuts=='Cancel'){
						$message =  '<b>Cancelled by Seller</b> : Your product of ORDER_'.$orderid.' has been Cancel, <a href="/artandcraft/dashboard/invoice.php?order='.$_GET['r'].'">Click to View</a>';
					}
            ...
            <div class="container-fluid">
                <div class="row">
					...
					<div class="panel panel-default">
							<div class="panel-heading"> Dashboard / View Orders</div>
							</div>
...
Order Id : ORDER_<?php echo base64_decode($_GET['order']); ?>
 ...
 // echo base64_decode($_GET['order']); die;
  ...
  <div class="row mb-5" style="margin-bottom: 34px;margin: 10px;">
	...
	<?php
	if($status == 1){
		echo '<p style="color:green;">Order Delivered Successfuly</p>';
	}else{
	?>
			<div class="dropdown1">
			  <button class="dropbtn1  btn btn-danger">Action</button>
			  <div class="dropdown-content1">
			  
			  <a href="<?php echo $BaseUrl.'/artandcraft/dashboard/order-invoice.php?orderPid='.$id.'&action=Shipped&r='.$_GET['order'].'&to_id='.$ppid.'&ppid='.$ppid;?>">Shipped</a>
			  <a href="<?php echo $BaseUrl.'/artandcraft/dashboard/order-invoice.php?orderPid='.$id.'&action=Out_Of_Delivery&r='.$_GET['order'].'&to_id='.$ppid.'&ppid='.$ppid;?>">Out Of Delivery</a>
			  <a href="<?php echo $BaseUrl.'/artandcraft/dashboard/order-invoice.php?orderPid='.$id.'&action=Delivered&r='.$_GET['order'].'&to_id='.$ppid.'&ppid='.$ppid.'&producid='.$spPostings_idspPostings;?>">Delivered</a>
			  <a href="<?php echo $BaseUrl.'/artandcraft/dashboard/order-invoice.php?orderPid='.$id.'&action=Cancel&r='.$_GET['order'].'&done_by=1&to_id='.$ppid.'&ppid='.$ppid;?>">Cancel</a>
			  
			  <!---       orderPid=$id       ----->
      <!----<a href="<?php echo $BaseUrl.'/artandcraft/dashboard/order-invoice.php?order='.$id.'&action=1';?>">Cancel Order</a>
			  <a href="<?php echo $BaseUrl.'/artandcraft/dashboard/my_order.php?order='.$id.'&action=2';?>">Return Request</a>
			  <a href="<?php echo $BaseUrl.'/artandcraft/dashboard/edit-order.php?order='.$id;?>">Edit Order</a>
			  <a href="<?php echo $BaseUrl.'/artandcraft/dashboard/order-invoice.php?order='.base64_encode($id);?>">View Order</a>--->
			  </div>
			</div>
	<?php } ?>
		</div>
