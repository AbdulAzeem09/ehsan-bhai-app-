...
<script src="<?php echo $BaseUrl; ?>/assets/js/posting/job-board.js"></script>
...
<input class="dynamic-pid" type="hidden" name="pid_" value="<?php echo $_SESSION['pid']; ?>" required>
...
<h3>
<i class="fa fa-pencil"></i> Post a Job 
<a href="<?php echo $BaseUrl.'/job-board/';?>"  style="color: #000;float:right ">&nbsp; | &nbsp;Back to Home </a>
<a href="<?php echo $BaseUrl.'/job-board/dashboard';?>" style="color: #000;float:right "> DASHBOARD  </a>
</h3>
</div>
</div>
</div>
...
<button class="btn btn-default dropdown-toggle" type="button" id="profiles" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true"><span class="<?php echo $icon; ?>"></span> <?php echo $name; ?><span class="caret"></span></button>
...
<p >
<small>Current Location: Belize, , <br>
...
/*print_r($row);*/
echo "<input type='hidden' id='postprofile' value='" . $row["idspProfiles"] . "' required>";
...
<input type="hidden" id="postid" value="<?php if (isset($postid)) { echo $postid; } ?>" required>
<input type="hidden" class="spCategories_idspCategory" name="spCategories_idspCategory"  value="<?php echo $_GET["categoryid"]; ?>" required>
<input type="hidden" id="catname"  value="<?php echo $_GET["categoryname"]; ?>" required>
<!--  <input type="hidden" id="buyid" name="buyid" type="hidden"> -->
<input type="hidden" id="spPostingVisibility" name="spPostingVisibility"  value="<?php echo (isset($_GET["groupid"]) ? $_GET["groupid"] : "-1"); ?>" required>
<input type="hidden" id="spPostingClosing" name="spPostingClosing"  value="0000-00-00" required>
<input type="hidden" id="spPostingEmail" name="spPostingEmail"  value="" required>
<input type="hidden" id="groupid" name="groupid"  value="0" required>
<!-- <input type="hidden" id="spPostingClosing" name="spPostingClosing"  value="0000-00-00">
<input type="hidden" id="spPostingClosing" name="spPostingClosing"  value="0000-00-00"> -->
<input type="hidden" id="spProfiles_idspProfiles" name="spProfiles_idspProfiles" class="business" value="<?php echo $_SESSION['pid']; ?>" required>
...
echo "<input id='idspPostings' name='idspPostings' value=" . $postid . " type='hidden' required>";
}
...
<div class="col-md-12">
<div class="form-group">
<label for="spPostingTitle" class="lbl_1">Job Title <span style="color:red;">*</span> &nbsp; <small>(Max 60 Character)</small>  <span id="errorTitle"></span></label> 
<input type="text" class="form-control" maxlength="60"  id="spPostingTitle" name="spPostingTitle"  value="<?php echo substr($ePostTitle, 0,60); ?>" placeholder="Your Posting Title" required>
</div>
</div>
<!--<div class="col-md-4">
<div class="form-group">
...
<div class="col-md-12">
<div class="form-group">
<label for="spPostingNotes" class="lbl_4">Job Description <span style="color:red;">*</span> <small>(Max 2000 Character)</small> <span id="errorDesc"></span></label>
<textarea class="form-control" maxlength="2000" id="spPostingNotes" name="spPostingNotes" rows="3" equired><?php echo $ePostNotes ?> </textarea>
</div>
</div>
<!--
<div class="col-md-6">
...
<input type='text' class='form-control' id='sppostingShippingCharge' name='sppostingShippingCharge' value='" . $shipping . "' required>
</div>
</div>";
echo "<div class='col-md-6'>
<div class='form-group'>
...
if(isset($_GET["repost"]) && $_GET['repost'] == 1){
?>
<button id="spPostSubmit" type="button" class="btn butn_save "><?php echo (isset($postid) ? "Repost" : "POST") ?></button>
<?php
}else{ ?>
<button id="spPostSubmitjob" type="button" class="btn butn_save "><?php echo (isset($postid) ? "POST" : "POST") ?></button>
...
if($postid && $postid>0){}else{ ?>

<button id="spSaveDraftJob" type="button" class="btn butn_draf">Save Draft</button>
<?php } ?>
<a href="<?php echo $BaseUrl.'/job-board/dashboard/active-post.php';?>" class="btn butn_cancel">Cancel Post</a>
...
header('Location: https://dev.thesharepage.com/job-board/');
