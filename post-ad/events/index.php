...
<style>
  .loadbox {
    z-index: 9999; /* Ensure loader appears above other elements */
  }
...
<h4 class="modal-title"  id="exampleModalLabel"><b style="color:black">Add Sponsor</b></h4>
</div>
...
<input type="number" class="form-control sponser_menu" id="spsponsorPrice" name="spsponsorPrice" placeholder="$ USD" maxlength="8" min="0" onkeyup="keyupsponsorfun()" />
...
<button id="addSponser" type="submit" class="btn btn-primary db_btn db_primarybtn btn-border-radius" style="background: #032350!important;">Add</button>
</div>
</form>
</div>
</div>
...
<button id="spaddSponsor" type="submit" class="btn btn-primary btn-border-radius">Add</button>
</div>
</form>
</div>
</div>
...
<button type="button" class="btn btn-danger btn-border-radius" data-dismiss="modal">Close</button>
<button type="button" class="btn btn-danger btn-border-radius" data-dismiss="modal">Close</button>
<button id="spaddexhibition" type="submit" class="btn btn-primary btn-border-radius">Create</button>
</div>
</form>
</div>
</div>
...
<h3>
<i class="fa fa-pencil" style="color:white;"></i><?php echo (isset($post_id) ? " Update an event " : " Submit an event ") ?>
...
<input id="spPostingVisibility" name="spPostingVisibility" type="hidden" value="<?php echo $visibility ?>">
...
<?php
if ($post_id) {
echo "<input id='idspPostings' name='idspPostings' value=" . $post_id . " type='hidden' >";
}
?>
<!--Art Gallery-->
...
<div class="col-md-6">
	<div class="form-group">
		<label for="ethnicity">Ethnicity<span style="color:red;">*</span><span id="ethnicity_error" class="label_error"></span></label>
		<select class="form-control sponser_menu" name="Ethnicity" id="ethnicity" onkeyup="keyupsponsorfun()">
			<option value="">Select Ethnicity</option>
			<option class="Indian" <?php echo (isset($ethnicity) && $ethnicity == 'Indian') ? 'selected' : ''; ?>>Indian</option>
			<option class="South Indian" <?php echo (isset($ethnicity) && $ethnicity == 'South Indian') ? 'selected' : ''; ?>>South Indian</option>
			<option class="Bollywood" <?php echo (isset($ethnicity) && $ethnicity == 'Bollywood') ? 'selected' : ''; ?>>Bollywood</option>
			<option class="African" <?php echo (isset($ethnicity) && $ethnicity == 'African') ? 'selected' : ''; ?>>African</option>
			<option class="Arabian" <?php echo (isset($ethnicity) && $ethnicity == 'Arabian') ? 'selected' : ''; ?>>Arabian</option>
			<option class="Bengali" <?php echo (isset($ethnicity) && $ethnicity == 'Bengali') ? 'selected' : ''; ?>>Bengali</option>
			<option class="East Asian" <?php echo (isset($ethnicity) && $ethnicity == 'East Asian') ? 'selected' : ''; ?>>East Asian</option>
			<option class="European" <?php echo (isset($ethnicity) && $ethnicity == 'European') ? 'selected' : ''; ?>>European</option>
			<option class="American" <?php echo (isset($ethnicity) && $ethnicity == 'American') ? 'selected' : ''; ?>>American</option>
		</select>
		<span id="sponsorCategory_error" style="color:red; margin-bottom: 0px;  font-size: 12px;"></span>
	</div>
</div>
...
<textarea class="form-control" style="display:none;" id="spPostingNotes" name="spPostingNotes" maxlength="5000"><?php echo $ePostNotes ?> </textarea>
<div id="editor-container" style=" height: 190px; "><?php echo $ePostNotes ?></div>
...
<script>
    ...
    quill1.on("text-change", function() {
		var editor_content = quill1.container.firstChild.innerHTML;
		//alert(editor_content);
		document.getElementById("spPostingNotes").value = editor_content;
		if (quill1.getLength() > 5000) {
			quill1.deleteText(5000, quill1.getLength());
		}
	});
</script>
</div>
...
<div class="row">
	<div class="col-md-12">
		<div class="form-group">
			<label for="spPostingReturnPolicy">Return Policy</label>
			<textarea class="form-control" style="display:none;" id="spPostingReturnPolicy" name="spPostingReturnPolicy" maxlength="1000"><?php echo $spPostingReturnPolicy ?> </textarea>
			<div id="editor-containers" style=" height: 190px; "><?php echo $spPostingReturnPolicy ?></div>
			<script>
				var quill2  = new Quill('#editor-containers', {
					modules: {
						toolbar: [
							[{
								header: [1, 2, false]
				...
				quill2.on("text-change", function() {
					var editor_contents = quill2 .container.firstChild.innerHTML;
					document.getElementById("spPostingReturnPolicy").value = editor_contents;
					if (quill2.getLength() > 1000) {
						quill2.deleteText(1000, quill2.getLength());
					}
				});
			</script>
		</div>
...
<label for="postingpic">Upload Poster(s) <span style="color:red;">*</span></label>
<input type="file" class="postingpic" id="filesaaa" name="spPostingPic">
<label id="lbl_55" class="label_error"></label>
<p class="help-block"><small>Browse files from your device</small></p>
<p class="help-block" style="color:red;"><small>Maximum allowed size 1500 KB</small></p>
...
<div class="row">
<div id="dvPreview" class="posterpreview">
<?php
//$i = 1;
$pic = new _eventpic;
if ($post_id) {
$res = $pic->read($post_id);
if ($res != false) {
while ($rows = mysqli_fetch_assoc($res)) {
...
echo "<div class='col-md-3 imagepost'><span class='fa fa-remove dynamicimg closed' style=''   data-work='event_1' data-aws='6' data-src='" . $rows['spPostingPic'] . "'  data-pic='" . $rows['idspPostingPic'] . "'></span><img class='overlayImage 111' width='150px' style='height: 120px; margin-right:5px;' data-name='fi_" . $i . "' src='" . ($picture) . "'/><label style='font-size: 10px;' class='updateFeature' data-postid='" . $post_id . "' data-picid='" . $rows['idspPostingPic'] . "'></div>";
$i++;
}
}
}
...
$id = $rp['idspPostings'];
if($pic2){
echo "<div class='col-md-2 imagepost'><span style='margin-right:10px;' class='fa fa-remove dynamicimg closed'  data-work='seatlayout' data-aws='6' data-src='" . $pic2 . "'  data-pic='" . $id . "'></span><img  id='layout' class='overlayImage 22' style='width:100%; height: 120px; margin-right:5px;' data-name='fi_" . $i . "' src='" . ($pic2) . "'/><label style='font-size: 10px;' class='updateFeature' data-postid='" . $post_id . "' data-picid='" . $rows['id'] . "'></div>";
}
}
} else {
}
...
<?php
$pic = new _eventpic;
$res2 = $pic->readGallery($post_id);

if ($post_id) {
if ($res2 != false) {
while ($rp = mysqli_fetch_assoc($res2)) {
$pic2 = $rp['image_name'];
echo "<div class='col-md-2 imagepost'><span style='' class='fa fa-remove dynamicimg closed'  data-work='event' data-aws='6' data-src='" . $pic2 . "'  data-pic='" . $rp['id'] . "'></span><img class='overlayImage 44' style='width:100%; height: 120px; margin-right:5px;' data-name='fi_" . $i . "' src='" . ($pic2) . "'/><label style='font-size: 10px;' class='updateFeature' data-postid='" . $post_id . "' data-picid='" . $rows['id'] . "'></div>";
}
} else {
}
}
...
<select id="rightmenu" class="sp_Sponsor form-control spPostField " name="sponsorId[]" multiple="multiple" style="width: 100%;" required>
...
<button type="button" class="btn btn-danger btn-border-radius" data-dismiss="modal">Close</button>
<a href="../../my-groups/" class="btn btn-default btn-border-radius" type="button">Add New</a>
</span>
</div>
</div>
<div class="col-md-8 text-right footer-button-block button_allign">
<?php
if ($post_id) {
/* echo "<a class='btn butn_draf' style='border-radius: 30px!important;' href='../deletePost.php?postid=" . $_GET['postid'] . "'>Delete post</a>";*/
}
?>
<a href="<?php if(isset($_SESSION['sign-up']) && $_SESSION['sign-up'] == 1) { echo $BaseUrl . '/registration-steps.php?pageid=8'; } else {echo $BaseUrl . '/events/dashboard';} ?>"  style="border-color: #d9534f;color: white;" class="btn btn-danger btn-border-radius">Cancel</a>
...
<?php if (!$post_id && (!isset($_SESSION['sign-up']) || $_SESSION['sign-up'] != 1)) { ?>
<button style="color: black;background-color:#ffb8e5" id="spSaveDraftevent" type="submit" class="btn butn_draf savedraftbtn btn-border-radius">Save As Draft</button>

<?php
}
if ($_GET['draft'] == 1) {
?>
<button id="spPostSubmitEvent" type="button" style="background-color:#c11f50!important" class="btn butn_save savesubmitbtn btn-border-radius <?php echo ((isset($post_id) && $post_id > 0) ? "editing" : ""); ?>"><?php echo ((isset($post_id) && $post_id > 0) ? "Publish" : "Submit") ?></button>
<?php
} else { ?>
<?php
...
<button id="<?php if($proffesionalAc == 1) { echo "spPostSubmitEventPro"; } else { echo "spPostSubmitEvent"; } ?>" type="button" style="background-color:#c11f50!important" class="btn butn_save savesubmitbtn btn-border-radius <?php echo ((isset($post_id) && $post_id > 0) ? "editing" : ""); ?>"><?php echo ((isset($post_id) && $post_id > 0) ? "Post" : "Submit") ?></button>
...
<?php } ?>
  ...
  <div id="myModals" style="display:none;" class="modal">
    <div class="modal-dialog" role="document">
      ...
      <div class="modal-footer">
        <button  id="spPostSubmitEvent" type="button" class="btn butn_save btn-border-radius">Submit
...
<?php 
if ($_GET['draft'] == 1) { ?>
<button id="spPostSubmitEventdraftupdate" type="button" style="background-color:#c11f50!important" class="btn butn_save savesubmitbtn btn-border-radius">Update</buttton>
<?php } ?>
...
<input type="hidden" class="form-control" id="spPostingExpDtevent" name="spPostingExpDtevent" value="<?php echo $expdate ?>">
</div>
