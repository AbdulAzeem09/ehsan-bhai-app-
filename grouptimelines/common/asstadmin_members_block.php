...
function generate_asst_admin_member_div($row){
    ...
    $memberAction = '';
    if($fstatus == "connect"){
        $memberAction = '<div id="conct_'.$profileid.'" class="link add_conct" data-prof="'.$profileid.'">
                    <span class="img">
    ...
    if($groupOwnerId != $profileid && $groupOwnerId == $_SESSION['pid'] || ($profileid !== $_SESSION['pid'])){
        $memberAction .= '<div id="astadm_'.$profileid.'" class="link rmv_asstadm" data-prof="'.$profileid.'">
                    <span class="img">
                        <img src="./images/diamonds.svg" alt="">
                    </span>
                    <span>Remove from Asst Admin</span>
                </div>
                <div id="blk_'.$profileid.'" class="link blk_prof" data-prof="'.$profileid.'">
                    <span class="img">
...
if ( $getAllAsstAdminMembersCount !='')  {                    
    while ($row = mysqli_fetch_assoc($getAllAsstAdminMembers)) {
        // echo "<pre>"; print_r($row);
        echo generate_asst_admin_member_div($row); 
    }
}
else {
    echo "No Assistant Admin."; 
}
