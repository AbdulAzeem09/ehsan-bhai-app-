...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $groupid = $row['spGroupMessage_idspGroupMessage'];
    $conversationid = $row['spGroupMessage_idspGroupMessage'];
//        echo "<pre>";
//        print_r($row);
//        echo "</pre>";
    echo'<tbody>';
...
echo '</table>';
echo '<form method="post" action="addconversation.php">';
echo '<input type="hidden" name="spGroupConProfile" value="' . $ProfileId . '"/>';
echo '<input type="hidden" id="spGroupMessage_idspGroupMessage" name="spGroupMessage_idspGroupMessage" value="' . $conversationid . '"/>			
			...
			<button class="addconversation" type="button" data-gmesgid="' . $conversationid . '">Send</button><br>';
echo '</form>';
//}
?>