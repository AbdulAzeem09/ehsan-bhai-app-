...
<div class="row">
<div class="col-md-6">
<span id="size1">Group  <small>[Active Events]</small></span>
...
$res = $p->getActiveEventsOfGroup($group_id, -1, 9 );
...
--->
<?php if ($row["spProfiles_idspProfiles"] == $_SESSION['pid']) { ?>
<!--<a href="<?php echo $BaseUrl.'/post-ad/events/group-form.php?groupid='.$row['spgroupid'].'&groupname=member testing&event&back=back&groupflag=gflag&postid='.$row['idspPostings']; ?>" class="" data-postid="<?php echo $row['idspPostings'];?>"><i class="fa fa-edit"></i></a>-->

<a href="<?php echo $BaseUrl.'/post-ad/events/?postid='.$row['idspPostings']; ?>" class="" data-postid="<?php echo $row['idspPostings'];?>"><i class="fa fa-edit"></i></a>

<a href="javascript:void(0)" class="delete-event" data-postid="<?php echo $row['idspPostings'];?>" data-groupId="<?php echo $group_id;?>">
...
$pst      = new _spgroup_event;
$res    = $pst->readsharePost($group_id);
// $p = new _spevent;
// $res = $p->myActPost($_SESSION['pid'], -1, $_GET["categoryid"]);
//$res = $p->getActiveEventsOfGroup($_GET["groupid"], -1, 9 );
//$res    = $p->publicpost_event($_GET["categoryID"]);
// echo $p->ta->sql;
//$i = 1; 
if($res != false){
...
function(isConfirm) {
if (isConfirm) {
$.post("../grouptimelines/delete-event.php", {eventId: evntId, groupId: groupId}, 
function (r) {
location.reload();
});
}
