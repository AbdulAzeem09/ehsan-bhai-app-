<?php
	...
	require 'email_campaign/Classes/PHPExcel/IOFactory.php';
			...
			<div class="row">
				...
				<div class="col-md-10">
					...
					<div class="panel panel-success">
						...
						<div class="panel-body previewdoc" style="background-color: #F0F0F0 ;">
							...
							<div class="collapse <?php echo (isset($_GET["message "])?"in":"")?>" id="messageboard">
								<div>	
									<h4><span class="title" style="font-size:20px;">Discussions</span><a href="#" style='float:right;' data-toggle="modal" data-target="#conversationModal"><span class="glyphicon glyphicon-plus"></span> New Discussions</a></h4>
									<?php
										$sendmessage = 2;
								...
								<?php
									...
									if ($result != false)
									{
										while($rw = mysqli_fetch_assoc($result))
										{
											...
											//<td width='7%'><button type='button' class='btn btn-link preview' data-toggle='modal' data-target='#previewfile' data-src='http://dev.thesharepage.com/resume/".$previewfile."' data-filetitle='".$rw['sppostingmediaTitle']."'><span class='glyphicon glyphicon-search'></span> Preview</button></td>
											
											$d=strtotime($rw['sppostingmediaDate']);
											$dt = new DateTime($rw['sppostingmediaDate']);
											 echo "<tr class='resumeoperation searchable'>
												<td width='72%' data-toggle='modal' data-target='#previewfile' data-src='http://dev.thesharepage.com/resume/".$previewfile."' data-filetitle='".$rw['sppostingmediaTitle']."' class='resumetitle preview'><a href='#'>".$title."</a></td>
												
												<td width='10%' class='searchtimelines fileuplaoder' data-profileid='".$rw['spProfiles_idspProfiles']."' data-profilename='".$rw['spProfileName']."' style='cursor:pointer;'><b style='color:gray;'>".$profilename."</b></td>
												
												
								                <td width='10%'>".$dt->format('d M Y')."  ".date("H:i:s", $d)."</td>
												
												<td width='2%'><a href='http://dev.thesharepage.com/resume/".$previewfile."'  data-mediaid='".$rw['idspPostingMedia']."' ata-toggle='tooltip' data-placement='left' title='Download'><span class='glyphicon glyphicon-download'  ></span></a></td>
		...
		<script type="text/javascript">
			...
			$('#saveEmail').on('click', function () {
			    ...
			    var optionValuetoo = $("#optionValuetoo").val();
			        ...
			        $.ajax({
			            type:'POST',
			            url:'http://dev.thesharepage.com/testing/grouptimelines/email_campaign/sendEmailCampaign.php',
			            data:{
			                'name':$('#txtName').val(),
				            // 'email':$('#email').val(),
				            'text':text,
