...
if (!isset($_SESSION['pid'])) {
	...
	<label><h3><a href='<?php echo $BaseUrl;?>/news/profile.php?id=<?php echo $Id;?>'><?php echo $row3['spProfileName']?></a><?php echo " Following"; ?></h3></label>
					...
					$all=$obj->readforallcount($Id);
					
					$allcount2=$all->num_rows;
					
					
					$ress=$obj->readfollowing($Id);
					 //print_r($ress);
					//
					if($ress!=false){
	  ...
	  $whom=$row['whom'];
		...
		if($_SESSION['pid']==$whom){
			...
			continue;
						...
						//die("00000");
						$res2=$obj->readfollowingdata($whom);
						if($res2!=false){
...
if($ress!=false){
 if($Id!=$whom){ 	
	...
	<h1 class="load-more2" style="text-align: center; font-size: 24px; cursor:pointer;" > Load More</h1>  
