...
$sqluserdata = mysql_query("SELECT email ,name, mobile_no from email_campaign_user WHERE id IN (99,104,101) ");
...
if ($campaignssql) {
    while ($campaigns = mysql_fetch_array($campaignssql)) {
        ...
        if ($user_group_type == 'group') {
            ...
            $sqlUserGroups = mysql_query("SELECT distinct user_id FROM user_groups WHERE group_id IN ($campaigngroupsIDs)");
        ...
        if ($userIds) {
            ...
            $sqluserdata = mysql_query("SELECT email ,name, mobile_no from email_campaign_user WHERE id IN ($usersIds) ");
            //$rs2 = @mysql_num_rows($sql2 > 0);
            if ($sqluserdata) {
                $mobile_no = array();
                $i = 0;
                ...
                if ($campaing_limit_ok) {
                    ...
                    $c_status = " Count of ".$i." is above user's remaining count".$count;
                    mysql_query("INSERT INTO sms_campaign_system_logs (campaign_id, status, date_time) VALUES ('" . $campaign_id . "', '" . $c_status . "', '" . $current_date_time . "')");
                }
            }
        } else {
