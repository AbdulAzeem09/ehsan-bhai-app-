	...
	if ($campaignssql) {
		while ($campaigns = mysql_fetch_array($campaignssql)) {
			...
			if ($user_group_type == 'importuser') {
				$sqlfileusers = mysql_query("SELECT * FROM email_campaign_user_groups WHERE campaign_id = '" . $campaigns['id'] . "'");

				while ($fileusersIds = mysql_fetch_array($sqlfileusers)) {
					$importIds[] = $fileusersIds['file_user_id'];
				}
			}
			...
			if ($userIds) {
				...
				if ($sqluserdata) {
					...
					//}
						...
						$user = "thesharepage"; //your username
						$password = "Jouple!2211"; //your password
				...
				$sqluserdata = mysql_query("SELECT name ,mobile_no FROM email_campaign_user WHERE id IN ($usersIds) ");
				if ($sqluserdata) {
					$mobile_no = array();
					$i = 0;
					while ($userdata = mysql_fetch_array($sqluserdata)) {
