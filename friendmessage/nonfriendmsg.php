...
<div class="panel panel-primary no-radius" style="border-color: #CCC">
		...
		<div class="row">
			...
			<div class="col-md-5">	
				<div class="form-group no-margin">
					<input type="text" class="form-control no-radius" id="srhfriend" placeholder="Search Friends...">
				</div>
			</div>
	...
	<div class="panel-body">
		...
		<div class="chattingsystem">
			<div style="height:449px; overflow-y: auto; overflow-x: hidden;" class="friend_message">
				<?php
					...
					$result = $message->nonfriendmsg($_SESSION["uid"] , $_POST["friendid"]);
					//echo $message->ta->sql;
					...
					if($result != false)
					{
						...
						while($rows = mysqli_fetch_assoc($result))
						{
							...
							echo "<tr class='message'>";
								echo "<td width='12%'><a href='#' data-toggle='tooltip' data-placement='left'  title='".($archived == 2 ?"Archived":"Not Archive")."' data-messageid='".$rows["idspfriendChatting"]."' class='".($archived == 2 ?"unarchive":"archive")."' style='".($archived  == 2 ?"color:yellow;":"color:#1a936f;")."' data-profileid='".$myid."'><span class='glyphicon glyphicon-floppy-save'></span></a>
								...
								echo "<td width='20%' class='commentoverflow'><b>".$sender."</b></td>";
								
								echo "<td  class='commentoverflow'>".$rows["spfriendChattingMessage"]."</td>";
		...
		<form method="post" id="myform">
			...
			<button class="sendmessagetofriend btn btn-success pull-right" type="button" >Send</button><br>
		</form>
	</div>
 </div>
</div>