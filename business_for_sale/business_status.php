	...
	<!-- Loader end -->
					...
					<div class="row">
						<div class="col-md-12">
							<div class="utf-main-search-container-area">
								...
								<form class="utf-main-search-form-item" action="business_status.php" method="get">
									<div class="row" style="margin-left:12%;">
										...
										<div class="col-sm-8 col-lg-8 custom-pl">
											<div class="category">
												...
												<a href="<?php echo $BaseUrl ?>/business_for_sale/business_status.php?page=1" class="reload_btn"><i class="fa fa-refresh" aria-hidden="true"></i></a>
											</div>
		...
		<section class="fullwidth" data-background-color="#f5f5f0">
			<div class="container">
				<div class="row">
					...
					if ($_GET['search']) {
						...
						$de2 = $de->read_business_status_id($_GET['id']);
					}
					//var_dump($de2);
					if ($de2 != false) {
						$st = mysqli_fetch_assoc($de2);
						//print_r($st);
						$status = $st['business_status'];
					}
					?>
					<div class="col-md-7">
						<h1><b><?php if ($status == 1) {
									echo "Business Status (For Sale)";
								} else if ($status == 2) {
									echo "Business Status (Under Offer)";
								} else {
									echo "Business Status (Sold)";
								}
					...
					<div class="col-md-12">
						<div class="carousel">
							...
							$de1 = $de->read_business_status($_GET['id'], $start, $limitaa);
							//print_r($de1);
							$num_rows = $de1->num_rows;
							if ($de1 != false) {
								while ($row = mysqli_fetch_assoc($de1)) {
									$st12 = new _spuser;
									$st1 = $st12->readdatabybuyerid($row['uid']);
									if ($st1 != false) {
														...
														<div class="row" style="padding-top: 10px;padding-left: 20px;">
															<label>
																<p style="line-height:17px;"><b>
																		...
																		<?php if ($row['business_category'] == 2) {
																			echo "Hotel";
																		} ?>
																	</b></p>
															</label><b>
																<?php echo $country; ?></b>,<b>
																<?php echo $city; ?></b>
														</div>
													</div>
												</div>
			...
			<?php if ($_GET['page'] != "1") { ?>
				...
				<a class="float-left btn btn-primary" href="<?php echo $BaseUrl . '/business_for_sale/business_status.php?page=' . $_GET['page'] - 1 & 'id=' . $_GET['id']; ?>">Previous</a>
			<?php  } ?>


			<?php if ($num_rows == "9") { ?>
				<a class="float-right  btn btn-primary" style="margin-bottom: 10px;" href="<?php echo $BaseUrl . '/business_for_sale/business_status.php?page=' . $_GET['page'] + 1 & 'id=' . $_GET['id']; ?>">Next</a>
			<?php  } ?>
		</section>
	...
	<div id="" style="background-color: #202447;color: white;">
		...
		<script>
			$("#search_btn").click(function() {
				...
				window.location.replace('<?php echo $BaseUrl ?>/business_for_sale/business_status.php?category=' + cat + '&search=' + data + '&page=1');
