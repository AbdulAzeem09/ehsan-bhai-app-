...
/*old?09/06/2020*/
  ...
  function singletimelinespost($postid) {
    ...
    function publicpost_jobBoardwithfilter($limit, $category, $startenddate, $jobtypefilter, $joblevelfilter, $Countryfilter, $Statefilter, $Cityfilter) {
        $this->ta->read("WHERE t.spPostingVisibility=-1 AND t.spPostingExpDt >= CURDATE() $startenddate $jobtypefilter $Countryfilter $Statefilter $Cityfilt $joblevelfilter AND t.spCategories_idspCategory = " . $category, "ORDER BY spPostingDate DESC LIMIT $limit");
   ...
   // MY TOTAL ACTIVE POST PROFILE VIESE
    ...
    function myProfilejobpost($pid) {
         ...
         return $this->ta->read("WHERE spProfiles_idspProfiles =" . $pid . " and spPostingVisibility = -1");
