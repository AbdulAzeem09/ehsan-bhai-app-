           ...
           <h2>Your current profile does not have <br>access to this page. Please create or switch<br> <span>"Business"</span> modules can post a job.</h2>
            <div class="space-md"></div>
             <a href="<?php echo $BaseUrl .
                 "/my-profile"; ?>" class="btn">Create or Switch Profile</a>
              <a href="<?php echo $BaseUrl .
                  "/job-board"; ?>" class="btn">Back to Home</a>
             </div>
           </div>
        </div>
    </div>
<?php if ($_SESSION["ptid"]) {
    $u = new _spuser();
...
<a href="<?php echo $BaseUrl .
    "/post-ad/job-board/?post"; ?>" class="btn butn_jobboard btn-border-radius">Post a job</a><?php } else { ?>
...
<option value='<?php echo $row3["country_id"]; ?>' <?php echo isset(
    $_SESSION["Countryfilter"]
) && $_SESSION["Countryfilter"] == $row3["country_id"]
    ? "selected"
...
<!-- <input type="text" class="form-control" id="spPostingCountry" name="spPostingsCountry" value="<?php echo isset(
    $eCountry
)
    ? $eCountry
    : $country; ?>"> -->
...
<?php if (isset($_SESSION["Statefilter"]) && $_SESSION["Statefilter"] > 0) {
    $countryId = $_SESSION["Countryfilter"];
    $pr = new _state();
    $result2 = $pr->readState($countryId);
    if ($result2 != false) {
        while ($row2 = mysqli_fetch_assoc($result2)) { ?>
<option value='<?php echo $row2["state_id"]; ?>' <?php echo isset(
    $_SESSION["Statefilter"]
) && $_SESSION["Statefilter"] == $row2["state_id"]
    ? "selected"
...
<?php if (isset($_SESSION["Cityfilter"]) && $_SESSION["Cityfilter"] > 0) {
    ...
    if ($result3 != false) {
        while ($row3 = mysqli_fetch_assoc($result3)) { ?>
<option value='<?php echo $row3["city_id"]; ?>' <?php echo isset(
    $_SESSION["Cityfilter"]
) && $_SESSION["Cityfilter"] == $row3["city_id"]
    ? "selected"
    : ""; ?>><?php echo $row3["city_title"]; ?></option> <?php }
    }
} ?>
</select>
<!-- <input type="text" class="form-control" id="spPostingCity" name="spPostingsCity" value="<?php echo isset(
    $eCity
)
    ? $eCity
    : $city; ?>"> -->
...
<input type="text" style="width: 490px;" name="txtJobTitle" id="txtJobTitle" class="form-control" value="<?php echo $_POST[
    "txtJobTitle"
...
<a style="border-color: #1f3060;background-color: #1f3060;    margin-right: 10px;padding: 10px 10px;" class="btn btn-primary pull-right btn-border-radius" href="<?php echo $BaseUrl .
    "/job-board/dashboard/emp_dashboard.php"; ?>">Dashboard</a>



<?php } else { ?>
<a style="border-color: #1f3060;background-color: #1f3060;    margin-right: 10px;padding: 10px 10px;" class="btn btn-primary pull-right zoom btn-border-radius" href="<?php echo $BaseUrl .
    "/job-board/dashboard"; ?>">Dashboard</a>

<?php }} ?>
<?php if ($_SESSION["guet_yes"] != "yes") { ?>
<!-- <a style="margin-right: 10px;" class="btn btn-primary pull-right" href="<?php echo $BaseUrl .
    "/job-board/forward-jobs.php?frw=1"; ?>">Forwarded Jobs</a> -->
<a style="margin-right: 10px;padding: 10px 10px;" class="btn btn-primary pull-right zoom btn-border-radius" href="<?php echo $BaseUrl .
    "/job-board/all-jobs.php"; ?>">All Jobs</a>
<?php } ?>
...
<li><a href="<?php echo $BaseUrl .
    "/job-board/search.php?level=" .
    $rows["masterDetails"]; ?>"><?php echo $rows["masterDetails"]; ?></a></li>
<?php
...
<div class="row">
<div class="col-md-2">
<input type="date" id="" name="fromdate" class="datepicker form-control" value="<?php echo $_GET[
    "fromdate"
]; ?>" placeholder="Start Date">
</div>
<!-- <div class="col-md-2">
<input type = "text" id = "end-date" name="todate" class="datepicker form-control" value="<?php echo $_GET[
    "todate"
...
$m = new _subcategory();
$catid = 2;
$result = $m->read($catid); /*echo $m->ta->sql;*/
if ($result) {
    while ($rows = mysqli_fetch_assoc($result)) { ?>
<option value='<?php echo $rows["subCategoryTitle"]; ?>' <?php if (
    isset($jobType)
) {
...
$Statefilter = "";
$Cityfilter = "";
$limit = "100"; /*$p   = new _postingview;*/
$p = new _jobpostings();
$pf = new _postfield();
$txtJobTitle = $_POST["txtJobTitle"]; //$txtJobLoc = $_POST['txtJobLoc'];
...
if (isset($_GET["searchforstorebtn"])) {
    if (!empty($_GET["fromdate"])) {
        ...
        $da = explode("/", $fromdate); //$da1=explode('/',$todate);
        $vdate1 = $da[2] . "-" . $da[1] . "-" . $da[0]; //$vdate2= $da1[2].'-'.$da1[1].'-'.$da1[0];
        $vdate1 = substr($vdate1, 2); // "quick brown fox jumps over the lazy dog."
        $startenddate = "AND spPostingDate LIKE '$vdate1%' "; //$startenddate = "AND spPostingDate BETWEEN '$vdate1' AND '$vdate2'";
    }
    ...
    $limit = "10000";
    $res = $p->publicpost_jobBoardwithfilter(
        $limit,
        2,
        $startenddate,
        $jobtypefilter,
    ...
    if (!empty($_POST["txtJobTitle"])) {
        ...
        if (
            !empty($_SESSION["Countryfilter"]) &&
            !empty($_SESSION["Statefilter"]) &&
            !empty($_SESSION["Cityfilter"])
        ) {
            if (isset($_SESSION["Countryfilter"])) {
                if (!empty($_SESSION["Countryfilter"])) {
                    $ccff = $_SESSION["Countryfilter"];
            ...
            $limit = -1; //$res = $p->publicpost_jobBoardwithfilter($limit, 2, $startenddate, $jobtypefilter, $joblevelfilter, $salaryrangefilter, $Countryfilter, $Statefilter, $Cityfilter);
            $res = $p->publicpost_jobBoard_session(
                $limit,
                $_SESSION["Countryfilter"],
                $_SESSION["Statefilter"],
                $_SESSION["Cityfilter"]
            ...
            $res_data = selectQ(
                "SELECT * from spjobboard where spPostingsCountry=? and spPostingVisibility=-1 and spPostingExpDt >= CURDATE()",
                "i",
                [$Countryfilter]
            );
        }
...
$dd = explode(" ", $state);
$dds = substr($dd[1], 0, 1); //echo $dds;
$ddh = substr($dd[2], 0, 1); //echo $ddh; //$state = $dds.$ddh; //echo $state; //die('===');
if ($job_count > 1) {
...
<?php //echo $p->ta->sql; die;
    ...
    if ($res) {
        while ($row = mysqli_fetch_assoc($res)) {
            ...
            } /*$postingDate = $p-> spPostingDate($row["spPostingDate"]);*/ // echo "<pre>";
            // print_r($row);
            $postingDate = $row["spPostingDate"]; //$cmpnyName = $row["spPostingDate"];
            $skill =
                $row[
                    "spPostingSkill"
            ...
            if ($account_status != 1) { ?>
				...
				<tr id="task-<?php echo $row[
        "idspPostings"
    ]; ?>" class="task-list-row" data-task-id="<?php echo $row[
    "idspPostings"
]; ?>" data-user="<?php echo $row[
    "spPostingJoblevel"
]; ?>" data-status="<?php echo $row[
    "spPostingJobType"
				...
				<a href="<?php echo $BaseUrl .
        "/job-board/job-detail.php?postid=" .
        $row["idspPostings"]; ?>" class="head-link">
				<div class="card-primary">
				<div class="col-md-9 col-sm-12">
				<div class="pri-head">
					<?php // Creates DateTime objects
     ...
     ?>
					<!-- <span class="head-days"><?php
                //echo round($diff / 86400);//
                ?> days left</span> -->
					<a href="<?php echo $BaseUrl .
         "/job-board/job-detail.php?postid=" .
         $row["idspPostings"]; ?>" class="head-link"><?php echo ucfirst(
    $row["spPostingTitle"]
); ?></a>
    ...
    if (strlen($string) > 65) {
        ...
        $endPoint = strrpos($stringCut, " "); //if the string doesn't contain any space then it will cut without word basis.
        $string = $endPoint
    ...
    ?>
     ...
     if (isset($usercityn) && $usercityn > 0) {
         $stateId = $currentstate_id;
         $co = new _city();
         $result3 = $co->readCity($stateId); //echo $co->ta->sql;
         if ($result3 != false) {
             while ($row3 = mysqli_fetch_assoc($result3)) {
     ...
     ?>
        ...
        echo $row["job_currency"] .
            " " .
            $row["spPostingSlryRngFrm"] .
            " - " .
            $row["job_currency"] .
            " " .
            $row["spPostingSlryRngTo"] .
            "";
    } ?>
				</div>
				<div class="avg-btn">
        ...
        $row[
            "idspPostings"
        ]; ?>" class="avg-bid btn zoom1 apl-nw">APPLY NOW </a>
				</div>
				<div class="location-btn">
				<a href="#" class="loc-btn"><?php echo ucfirst(
        $row["spPostingLocation"]
    ); ?> </a>
				</div>
				</div>
				</div>
    ...
    if ($res_data) {
        foreach ($res_data as $row) {
            ...
            if ($flagnums == "9") {
                $updatestatus = $pf->jobboardstatus($idposting);
            } /*$postingDate = $p-> spPostingDate($row["spPostingDate"]);*/ // echo "<pre>"; // print_r($row);
            $postingDate = $row["spPostingDate"]; //$cmpnyName = $row["spPostingDate"];
            $skill =
                $row[
                    "spPostingSkill"
...
<h1>Trending Companies (<a href="<?php echo $BaseUrl .
    "/job-board/trend-cmpany.php"; ?>">View All</a>)</h1>
<div class="space-lg"></div>
<?php
$limitpr = 3;
$p = new _jobpostings();
...
if ($result3) {
    while ($row3 = mysqli_fetch_assoc($result3)) {
        ...
        $result5 = $p->readOpenJobs($row3["idspProfiles"]); //echo $p->ta->sql;
        if ($result5) {
...
if ($result4 != false) {
    ...
    if (isset($row4["spProfilePic"])) {
        echo "<img alt='profile pic' class='img-responsive' src=' " .
            $row4["spProfilePic"] .
            "'  >";
    } else {
        echo "<img alt='profilepic' class='img-responsive' src='" .
            $BaseUrl .
            "/assets/images/icon/blank-img.png' >";
    }
}
?>
<div class="">
<p class="titlejob">
<a href="<?php echo $BaseUrl .
    "/job-board/company.php?cmpyid=" .
    $row4["idspProfiles"]; ?>"><?php echo $cmpnyName; ?></a>
<span class="pull-right">
<a href="<?php echo $BaseUrl .
    "/job-board/company.php?cmpyid=" .
    $row4["idspProfiles"]; ?>"><?php echo $totalJob; ?> Position openings </a>
</span>
</p>
<p class="postingng">Company Size: <?php echo $CmpnySize == ""
    ? "Not Define"
    : $CmpnySize; ?></p>
</div>
</div><?php
...
<div class="col-md-6 right_left_jobboard">
<h1>Recent Job Offers (<a href="<?php echo $BaseUrl .
    "/job-board/all-jobs.php"; ?>">View All</a>)</h1>
<div class="space-lg"></div>
<?php
$limit = 3;
$p = new _jobpostings();
$pf = new _postfield();
$res = $p->publicpost_jobBoard($limit, 2); //echo $p->ta->sql;
if ($res != false) {
    while ($row = mysqli_fetch_assoc($res)) {
...
if ($result4 != false) {
    ...
    if (isset($row4["spProfilePic"])) {
        ...
        echo "<img alt='profilepic' class='img-responsive' src='" .
            $BaseUrl .
            "/assets/images/icon/blank-img.png' style='display:inline'; >";
    }
}
?>
...
<div class="">
<p class="titlejob"><a href="<?php echo $BaseUrl .
    "/job-board/job-detail.php?postid=" .
    $row["idspPostings"]; ?>"><?php echo $row[
    "spPostingtitle"
]; ?></a> <span class="pull-right">Apply</span></p>
...
<!--<a href="<?php echo $BaseUrl .
    "/job-board/all-jobseeker.php?cat=ALL&offset=0"; ?>" >Hire Today</a>-->
<a href="<?php echo $BaseUrl .
    "/job-board/all-jobseeker.php?cat=ALL&offset=0"; ?>">Hire Today</a>
</div>
</div>
<?php } elseif ($_SESSION["ptid"] != 1) { ?>
<div class="col-md-offset-1 col-md-10 no-padding">
...
<?php
$pro = new _spprofiles(); //die('=gggg===');
$result7 = $pro->profileTypePerson(5, $_SESSION["uid"]); //echo $pro->ta->sql;
if ($result7 != false) {
...
<div class="avatar">
<a href="<?php echo $BaseUrl .
    "/job-board/user-profile.php?pid=" .
    $rows["idspProfiles"]; ?>">
<?php
$picture = $rows["spProfilePic"];
if (isset($picture)) {
    echo "<img  alt='Posting Pic' class='img-responsive' src=' " .
        $picture .
        "' >";
} else {
...
<h5 class="freelancer-name" id="jobseaker-name"><a href="<?php echo $BaseUrl .
    "/job-board/user-profile.php?pid=" .
    $rows["idspProfiles"]; ?>"><?php echo $rows["spProfileName"]; ?></a></h5>
<?php
$fi = new _profilefield();
$result_fi = $fi->getType($rows["idspProfiles"]); //echo $fi->ta->sql;
if ($result_fi) {
    $row_fi = mysqli_fetch_assoc($result_fi);
    $pro = new _projecttype();
    $result_pro = $pro->getProjectName($row_fi["spProfileFieldValue"]); //echo $pro->ta->sql;
    if ($result_pro) {
...
if ($result_sk) {
    ...
    $totalSkil = count($string_sk); //print_r($string_sk);
} else {
    $totalSkil = 0;
}
?>
