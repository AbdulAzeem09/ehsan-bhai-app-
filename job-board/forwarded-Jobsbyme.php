...
$result = $fwrd->delforwardjobsme($deleteid);
...
//print_r($_SESSION);
$fwrd = new _forwardjobs;
$result = $fwrd->readFwrdJobbyme($_SESSION['pid']);
...
<td><a href="/job-board/user-profile.php?pid=<?php echo $row['frwReciverId']; ?>"><?php 
$prs = new _spprofiles;
$result12 = $prs->read($row['frwReciverId']);
if($result12!=false){			
$resprofile = mysqli_fetch_array($result12);
echo $resprofile['spProfileName'];
}

$href = '/job-board/forwarded-Jobsbyme.php?deleteid=' . $row['frwId'];
?>
</a></td>
<td>
<a onclick="permanentDelete('<?php echo $href ?>')"  class="disable-btn"><i style="color:red;" class="fa fa-trash disable-btn" data-disableid=""></i></a>
</td>
</tr>
<?php } }?>	
</tbody>
