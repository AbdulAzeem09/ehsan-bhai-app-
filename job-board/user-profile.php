...
<div class="row">
<div class="col-md-3">
<h4> <a href="<?php echo $BaseUrl; ?>/job-board/all-jobseeker.php?cat=ALL&offset=0" style="color: white;border-radius: 2px;background-color: #428bca;
padding: 8px;"><i class="fa fa-arrow-left" aria-hidden="true"></i>  Back</a></h4>
<?php
...
<input type="hidden" name="receiver_idspProfiles" value="<?php echo $pid; ?>" >
...
<input type="hidden" name="txtReceiver" id="txtReceiver" value="<?php echo $pid; ?>">
<div class="form-group">
<label>To (<?php echo ucfirst($Title);?>)<span class="red"> * <span class="error_user"></span></span></label>
...
<div class="row text-right"  style="cursor: pointer;">

<a href="javascript:void(0)" style="padding-left: 5px;" id="composeNewTxt" class="btn btn-primary" data-toggle="modal" data-target="#composeNewTxt">Message</a>
...
$flagp = new _flagpost;
$flagdetail = $flagp->readmyflagPro_d($userid,$profid);
$flagdetail->num_rows;
if($account_status!=1){
if($flagdetail != false){
$row = mysqli_fetch_array($flagdetail);
$id = $row['id'];
}
}
?>
...
<!--<option value="2">Send profile a message</option>-->
<option value="3">Download Resume</option>
<!--<option value="4">Recommend to someone</option>-->
<?php if($flagdetail){?>

<option value="1">Unflag Profile</option>

<?php }else{?>
<option value="5">Flag Profile</option>
<?php } ?>
</select>
</div>
</div>
<br>
<?php
if($_GET['action'] == "delflag"){  
$profid= $pid;
$flagid =	$row['id'];;
$flagpro = new _flagpost;
$data=$flagpro->deleteflag($flagid);
...
position: 'top-center',
icon: 'success',
title: 'Succesfully Unflaged!',
showConfirmButton: false,
timer: 1500
})</script>
<script>
<script>
window.location.replace('<?php echo $BaseUrl?>/job-board/user-profile.php?pid=<?php echo $profid?>');
</script>
<?php	}
?>
<?php
...
if($da->num_rows > 0) {
echo "<script>alert('you have Successfully flagged this post from another profile');</script>";
} else {
...
<h4 class="heading12">Personal Information333</h4>
</div>----->
...
var value = $(this).val();
if(value=="3"){
window.location= "<?php echo $BaseUrl; ?>/job-board/mpdffiles.php?pid=<?php echo $pid; ?>";
//window.print();
}
else if(value=="5"){

//	$(location).prop('href', 'https://dev.thesharepage.com/job-board/user-profile.php?pid=1613');
$(location).prop('href', '<?php echo "https://" . $_SERVER['SERVER_NAME']; ?>/job-board/user-profile.php?action=flag&pid=<?php echo $pid; ?>');
...
chatWith(<?php echo $pid;?>);
}
else if(value=="1"){
$(location).prop('href', '<?php echo "https://" . $_SERVER['SERVER_NAME']; ?>/job-board/user-profile.php?action=delflag&pid=<?php echo $pid; ?>');   
}
