	...
	include '../../univ/baseurl.php';
        ...
        $r = new _rfq;
         $result = $r->readrfqprofile($_POST['spProfiles_idspProfiles']);
...
$result1 = $r->readRfqComment($row['idspRfq']);
		...
		$rfqdata[] = array("idspRfq"=>$row['idspRfq'],"rfqTitle"=>$row['rfqTitle'],"rfqCategory"=>$row['rfqCategory'],"rfqQty"=>$row['rfqQty'],"rfqDelivered"=>$row['rfqDelivered'],"rfqDesc"=>$row['rfqDesc'],"spProfiles_idspProfiles"=>$row['spProfile_idspProfiles'],"NameOfProfile"=>$NameOfProfile,"rfqDate"=>$row['rfqDate'],"spQuotereached"=>$Quatationcount,"rfqprice"=>$row['rfqprice'],"rfqImage"=>$BaseUrl.'/upload/store/rfq/'.$row['rfqImage'],'rfqCountry'=>$row['rfqCountry'],'countryname' => $country,'rfqState'=>$row['rfqState'],'statename' => $state,'rfqCity'=>$row['rfqCity'],'cityname' => $city);
