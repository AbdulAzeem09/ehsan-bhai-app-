...
if ((!isset($BaseUrl)) OR (empty($BaseUrl))) {
	if(isset($_SERVER['SCRIPT_FILENAME'])) {
		$BaseUrl = str_replace( '\\', '/', substr($_SERVER['SCRIPT_FILENAME'], 0, 0-strlen($_SERVER['PHP_SELF'])));
	} elseif(isset($_SERVER['PATH_TRANSLATED'])) {
		$BaseUrl = str_replace( '\\', '/', substr(str_replace('\\\\', '\\', $_SERVER['PATH_TRANSLATED']), 0, 0-strlen($_SERVER['PHP_SELF'])));
	} else {
...
if (!defined('K_PATH_URL')) {
	...
	if (isset($_SERVER['HTTP_HOST']) AND (!empty($_SERVER['HTTP_HOST']))) {
		...
		$k_path_url .= str_replace( '\\', '/', substr(K_PATH_MAIN, (strlen($BaseUrl) - 1)));
	}
