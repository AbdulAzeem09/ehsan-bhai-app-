document.addEventListener("DOMContentLoaded", function () {
  ...
  if (document.getElementById("freelancearrow")) {
    ...
    const freelancerrotateButton = document.getElementById(
      "freelancearrowbutton"
    );
  ...
  function copyToClipboard(text) {
    navigator.clipboard
      .writeText(text) // Use Clipboard API to write text to clipboard
      .then(() => {
    ...
    if (input.files && input.files[0]) {
      if (input.files[0].type.indexOf("video/") === 0) {
        ...
        if (input.files[0].size > 52428800) {
          Swal.fire(
            "Video file is too big. Please select a file smaller than 50MB."
          );
          input.value = "";
        }
        ...
        if (input.files[0].size > 52428800) {
          Swal.fire(
            "Audio file is too big. Please select a file smaller than 50MB."
          );
          input.value = "";
        }
      }
  ...
  window.validateDocumentSize = function () {
    ...
    if (input.files && input.files[0]) {
      ...
      if (input.files[0].size > maxAllowedSize) {
        Swal.fire(
          "Document size is too big, Please select a file that is less than  5MB."
        );
        input.value = "";
      }
    }
    ...
    if (fileList.files.length > 0) {
      for (var i = 0; i <= fileList.files.length - 1; i++) {
        ...
        if (fsize > maxAllowedSize) {
          Swal.fire(
            "Image size is too big, Please select a file that is less than  5MB."
          );
          fileList.value = "";
        }
      }
  ...
  let emailVerificationStep = 1;
  let newEmailForVerification = '';

  const firstVerifyBtn = document.getElementById('first_verify');
  if (firstVerifyBtn) {
    firstVerifyBtn.addEventListener('click', function(event) {
      event.preventDefault();
      addNewEmail(event);
    });
  }

  const firstOtpVerifyBtn = document.getElementById('first_otp_verify');
  if (firstOtpVerifyBtn) {
    firstOtpVerifyBtn.addEventListener('click', function(event) {
      console.log('First OTP verify button clicked');
      event.preventDefault();
      verifyFirstOTP(event);
    });
  }

  const sendAgain1 = document.getElementById('send-again1');
  if (sendAgain1) {
    sendAgain1.addEventListener('click', function() {
      console.log('Send again 1 clicked');
      resendOTP('first');
    });
  }

  const sendAgain2 = document.getElementById('send-again2');
  if (sendAgain2) {
    sendAgain2.addEventListener('click', function() {
      console.log('Send again 2 clicked');
      resendOTP('second');
    });
  }

  const secondVerifyBtn = document.getElementById('second_verify');
  if (secondVerifyBtn) {
    secondVerifyBtn.addEventListener('click', function() {
      console.log('Second verify button clicked');
      verifySecondOTP();
    });
  }

  // Handle Enter key on OTP inputs
  document.addEventListener('keypress', function(event) {
    if (event.key === 'Enter') {
      const registerOtp = document.getElementById('register_otp');
      if (registerOtp && registerOtp === document.activeElement) {
        console.log('Enter key pressed on first OTP input');
        verifyFirstOTP(event);
        event.preventDefault();
      }
      
      const newOtp = document.getElementById('new_otp');
      if (newOtp && newOtp === document.activeElement) {
        console.log('Enter key pressed on second OTP input');
        verifySecondOTP(event);
        event.preventDefault();
      }
    }
  });
...
$(document).ready(function () {
  ...
  $("#spPostSubmitTimeline").on("click", function (ev) {
    ...
    if (grptimelinefrmtxt != "") {
      ...
      if (strArr[0] == " ") {
        // this is the the key part. you can do whatever you want here!
        flag = 1;
      }
    }
    if (
      ...
      if (idspprofile != "") {
        ...
        term.push({ name: "spPostingNotes", value: grptimelinefrmtxt });
        ...
        .done(function (data) {
          ...
          if ($("#catname").val() != "") {
            ...
            Swal.fire({
              ...
              text:
                "View Your <a href='../post-details/?postid=" +
                postid +
                "' style='color:#F8BB86'>Post</a>",
              html: true,
            });
          }
          // IMAGE
          ...
          $(".postingimg").each(function (i, e) {
            ...
            if (str1.indexOf(str2) != -1) {
              form_data.append(
                "spPostingPic",
                $("input[type=file]")[1].files[i]
              );
            } else {
              form_data.append(
                "spPostingPic",
                $("input[type=file]")[0].files[i]
              );
            }
            $.ajax({
            ...
            if (file[0].size <= 20971520) {
              if (regex.test(file[0].name.toLowerCase())) {
                ...
                reader.onload = function (e) {
                  ...
                  if (r == 1 && imgcound > 0) {
                    var img = $(
                      "<div class='col-md-2 imagepost'><span class='fa fa-remove dynamicimg 222 closed'></span><img class='postingimg overlayImage11' style='width:100px; height: 100px; margin-right:5px;' data-name='fi_" +
                        countNum +
                        "' src='" +
                        e.target.result +
                        "'/><label style='font-size: 9px;'><input type='hidden' class='featureImg' name='featureImg_' id='fi_" +
                        countNum +
                        "' value='1' />Feature Image</label></div>"
                    );
                  } else if (r == 1) {
                    var htmlContent =
                      "<div class='col-md-2 imagepost'><span onclick='removeImage()' class='fa fa-remove dynamicimg closed' style='margin-right:4px !important;'></span><img class='postingimg overlayImage22' style='width:100px; height: 100px; margin-right:5px;' data-name='fi_" +
                      countNum +
                      "' src='" +
                      e.target.result +
                      "'/>";
                    if (showFea === "1") {
                      htmlContent =
                        htmlContent +
                        "<label style='font-size: 9px;'><input type='radio' class='featureImg' name='featureImg_' id='fi_" +
                        countNum +
                        "' value='1' checked />Feature Image</label>";
                    }
                    htmlContent = htmlContent + "</div>";
                    var img = $(htmlContent);
                  } else {
                    htmlContent =
                      "<div class='col-md-2 imagepost'><span class='fa fa-remove dynamicimg 444 closed'></span><img class='postingimg overlayImage33' style='width:100px; height: 100px; margin-right:5px;' data-name='fi_" +
                      countNum +
                      "' src='" +
                      e.target.result +
                      "'/>";
      ...
      $($(this)[0].files).each(function () {
        ...
        if (file[0].size <= 5097152) {
          ...
          if (
            ...
            var file_name =
              file[0].name + " is not a valid file. upload only documents";
            Swal.fire({
    ...
    if (typeof FileReader != "undefined") {
      ...
      $($(this)[0].files).each(function () {
        ...
        if (file[0].size <= 52428800) {
          if (
            file[0].name.match(
              /.(mp4|3g2|3gp|avi|flv|f4v|h264|m4v|mkv|html5|mov|mp4|mpeg-2|mpg|rm|swf|vob|wmv|mp3|mpa|webm|avchd|mp2|3gp|mpeg|mpe|wav|mpv|ogg|wma|mts|mt2s|ts|qt|m4p|midi|mid)$/i
            )
          ) {
            var reader = new FileReader();
            $("#mediaTitle").html(fileName);
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.ajax({
        ...
        success: function (response) {
          $("#savefun" + id).html('<span class="dropdown-item"  onclick="myUnsave(' +
              id +
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.ajax({
        ...
        success: function (response) {
          $("#savefun" + id).html(
            '<span class="dropdown-item" class="profile_section" onclick="savePosts(' +
              id +
...
function addDiv(item) {
  ...
  if (profileid == item.spProfiles_idspProfiles) {
    html +=
      ...
      '<span class="dropdown-item edit-post" data-postid="' + item.idspPostings + '"><i class="fa fa-pencil-alt" ></i></span></span>' +
      "</li>" +
      '<li style="cursor:pointer;">' +
      '<span class="dropdown-item" onclick="deletePost(' +
      item.idspPostings +
...
if ($("#grptimelinefrmtxt").length) {
  ...
  var maxEmojis = 10; // Set the maximum number of emojis allowed
  ...
  // Add the remaining character count below the editor
  var charCountElem = $("#char-count"); // Select the character count element

  // Emoji Regular Expression to match emoji characters
  const emojiRegex = /(\p{Emoji_Presentation}|\p{Extended_Pictographic})/gu;

  // Monitor text changes in the editor
  quillFull.on("text-change", function (delta, oldDelta, source) {
    var text = quillFull.getText().trim(); // Get editor text and remove whitespace
    var emojiMatches = text.match(emojiRegex) || []; // Get all emojis in text

    // Calculate number of emojis
    var currentEmojiCount = emojiMatches.length;

    // Calculate remaining characters
    var remainingChars = Math.max(maxChars - text.length, 0);

    // Update character count display
    charCountElem.text(
      `${remainingChars} characters remaining | ${
        maxEmojis - currentEmojiCount
      } emojis remaining`
    );

    // Check if emoji limit is exceeded
    if (currentEmojiCount > maxEmojis) {
      // Remove the last inserted emoji to keep within the limit
      let lastChange = delta.ops[delta.ops.length - 1];

      if (
        lastChange &&
        lastChange.insert &&
        typeof lastChange.insert === "string"
      ) {
        quillFull.deleteText(
          quillFull.getLength() - lastChange.insert.length - 1,
          lastChange.insert.length
        );
      }
      charCountElem.css("color", "red"); // Show in red when limit is exceeded
    } else {
      // Apply default color styling if within limits
      charCountElem.css("color", "gray");
    }

    // Check if character limit is exceeded
    if (text.length > maxChars) {
      quillFull.deleteText(maxChars, text.length);
    }
  });
...
/**
 * Add new email and send OTP to primary email
 */
function addNewEmail(event) {
  event.preventDefault()
  const personalEmail = document.getElementById('personalEmail');
  const errorElement = document.getElementById('error-personalEmail');
  ...
  if (!emailPattern.test(personalEmail.value)) {
    if (errorElement) {
  ...
  newEmailForVerification = personalEmail.value;

  console.log('Adding new email:', newEmailForVerification);

  // Call EditProfile.sendOtp() with type='first' to send OTP to primary email
  $.ajax({
    type: 'POST',
    url: "/api.php?class=EditProfile&action=sendOtp",
    data: {
      class: 'EditProfile',
      function: 'sendOtp',
      type: 'first',
      mail: newEmailForVerification
    },
    dataType: 'json',
    success: function(res) {
      let response = res;
      console.log('Response from sendOtp (first):', response);
      ...
      if (response === 'Success') {
        console.log('OTP sent to primary email successfully');
        // Show OTP input within .first section
        const emailInputGroup = document.querySelectorAll('.first .input-group')[0];
        const otpInputGroup = document.querySelectorAll('.first .input-group')[1];
        const sendAgainLabel = document.querySelector('.first > .label.w-100');
        const line = document.querySelector('.first > .line');
        
        if (emailInputGroup) emailInputGroup.classList.add('hidden');
        if (otpInputGroup) otpInputGroup.classList.remove('hidden');
        if (sendAgainLabel) sendAgainLabel.classList.remove('hidden');
        if (line) line.classList.remove('hidden');
        
        emailVerificationStep = 2;
        toastr.info('OTP sent to your primary email. Please verify.');
      } else {
        if (errorElement) {
          errorElement.textContent = response.error || 'Failed to send OTP.';
        }
        toastr.error(response.error || 'Failed to send OTP.');
        console.error('Failed to send OTP:', response);
      }
    },
    error: function(xhr, status, error) {
      console.error('Error sending OTP:', status, error, xhr.responseText);
      if (errorElement) {
        errorElement.textContent = 'Error sending OTP. Please try again.';
      }
...
/**
 * Verify first OTP sent to primary email
 */
function verifyFirstOTP() {
  const registerOtp = document.getElementById('register_otp');
  const errorElement = document.getElementById('error-first_verify');

  console.log('Verifying first OTP');

  if (!registerOtp || !registerOtp.value.trim()) {
    if (errorElement) {
  ...
  // Call EditProfile.sendOtp() with type='second' to verify primary email and send to new email
  $.ajax({
    type: 'POST',
    ...
    data: {
      ...
      type: 'second',
      mail: newEmailForVerification,
      code: registerOtp.value
    },
    dataType: 'json',
    success: function(response) {
      console.log('First OTP verification response:', response);
      if (response.data === 1) {
        console.log('Primary email verified, OTP sent to new email');
        // Move to show second step - OTP input for new email
        toggleEmailVerificationSteps('second');
        emailVerificationStep = 3;
        registerOtp.value = '';
        toastr.success('Primary email verified! Sending OTP to new email...');
      } else {
        if (errorElement) {
          errorElement.textContent = response.message || 'Invalid OTP.';
        }
    ...
    error: function(xhr, status, error) {
      console.error('Error verifying first OTP:', error, xhr.responseText);
      if (errorElement) {
        errorElement.textContent = 'Error verifying OTP. Please try again.';
      }
      toastr.error('Error verifying OTP. Please try again.');
    }
  });
}

/**
 * Verify second OTP sent to new email
 */
function verifySecondOTP() {
  const newOtp = document.getElementById('new_otp');
  const errorElement = document.querySelector('.second .error-message');

  console.log('Verifying second OTP');

  if (!newOtp || !newOtp.value.trim()) {
    if (errorElement) {
      errorElement.textContent = 'Please enter the OTP.';
    }
    return;
  ...
  // Call EditProfile.sendOtp() with type='third' to verify new email
  $.ajax({
    type: 'POST',
    url: window.location.pathname,
    data: {
    ...
    dataType: 'json',
    success: function(response) {
      console.log('Second OTP verification response:', response);
      if (response.data === 1) {
        // Show success message
        toggleEmailVerificationSteps('success');
        emailVerificationStep = 4;
        newOtp.value = '';
        showContinueButton();
    ...
    error: function(xhr, status, error) {
      console.error('Error verifying second OTP:', status, error, xhr.responseText);
      if (errorElement) {
        errorElement.textContent = 'Error verifying OTP. Please try again.';
      }
      toastr.error('Error verifying OTP. Please try again.');
 ...
 * Resend OTP
 */
function resendOTP(step) {
  console.log('Resending OTP for step:', step);
  
  if (step === 'first') {
    // Resend OTP to primary email
    $.ajax({
      type: 'POST',
      url: window.location.pathname,
      data: {
      ...
      dataType: 'json',
      success: function(response) {
        console.log('Resend first OTP response:', response);
        if (response.data === 'Success') {
          toastr.success('OTP resent to your primary email.');
        } else {
    ...
    $.ajax({
      ...
      dataType: 'json',
      success: function(response) {
        console.log('Resend second OTP response:', response);
        if (response.data === 1) {
          toastr.success('OTP resent to your new email.');
        } else {
          toastr.error(response.message || 'Failed to resend OTP.');
        }
      },
...
/**
 * Toggle email verification step visibility
 */
function toggleEmailVerificationSteps(showClass, hideClass) {
  ...
  const elementToShow = document.querySelector('.' + showClass);
  if (elementToShow) {
    elementToShow.classList.remove('hidden');
    console.log('Showing element:', showClass);
  }
}
