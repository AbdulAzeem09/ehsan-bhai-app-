...
<h2>My Earnings - Arts and crafts Module</h2>
</div>
<?php
$u = new _spuser;
...
if ($res1 != True) {
?>
<button type="button" class="btn btn-primary pull-right btn-border-radius" data-toggle="modal" data-target="#exampleModal" style="margin-right:25px">
Withdraw Amount
</button>
<?php }
} ?>
...
<!--<a href="https://dev.thesharepage.com/dashboard/finance/artandcraft.php">Lifetime Sale </a>-->
...
// die("------------");
$result = $pw->readartandcraft($_SESSION['uid']);
...
$pf = new _spprofiles;
$result1 = $pf->profilenameant($buyer_userid);
//print_r($row1);die('=====');
if ($result1) {
...
<!--<td   class="text-center "><span class="smalldot"><?php if ($row['status'] == 0) {
echo 'hold';
} else {
echo 'done';
} ?></span></td>-->
...
<input type="hidden" name="store" value="artandcraft">
...
<input class="form-control" type="number" min="50" max="<?php if ($amount3) {
echo $amount3;
} else {
echo $amount1;
} ?>" placeholder="Minimum 50 - Maximum <?php if ($amount3) {
echo $amount3;
} else {
echo $amount1;
