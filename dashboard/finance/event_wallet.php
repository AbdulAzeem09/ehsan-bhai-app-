...
<h2>My Earnings - Event Module</h2>
</div>
...
<p class="no-margin" >
<i class="fa fa-info-circle"></i>Your mobile number is not yet verified. Kindly verify your mobile number.
...
<button type="button" class="btn btn-primary pull-right btn-border-radius" data-toggle="modal" data-target="#exampleModal" style="margin-right:35px;margin-top: -29px;">
Withdraw Amount
</button>
...
<!--  <a href="https://dev.thesharepage.com/dashboard/finance/event_wallet.php">Lifetime Sale </a>-->

<h4 style="font-weight:bold;">LIFETIME SALE </h4>
...
<h4 style="font-weight:bold;">TOTAL WITHDRAWAL</h4>
</div>
</div>
</div>
...
<h4 style="font-weight:bold;">TOTAL AMOUNT LEFT</h4>
</div>
</div>
</div>
...
$pw = new _spevent_transection;
// die("------------");
$result = $pw->read_wallet($_SESSION['uid']);
//var_dump($result);
...
<?php 
if(isset($_POST['spBankusername']))
{

  if(isset($_SESSION['withdraw_otp_success']) == 1){
    unset($_SESSION['withdraw_otp_success']);
...
<?php
$w = new _orderSuccess;
$waa = $w->readuser($_SESSION['uid']);
if ($waa) {
  $userData = mysqli_fetch_assoc($waa);
  $amount = $userData['amount'];
  $spBankusername = $userData['spBankusername'];
  $spBankname = $userData['spBankname'];
  $spBanknumber = $userData['spBanknumber'];
  $spBranchnumber = $userData['spBranchnumber'];
  $spAccountnumber = $userData['spAccountnumber'];
  $spBankcode = $userData['spBankcode'];
}
?>
<?php
...
You have Request Withdraw on (date <?php echo $dated; ?>) Please wait !
</div>
...
<form action="" method="post" id="withdrawalForm" >
  ...
  <div class="row">
    ...
    <div class="col-md-8">
      ...
      <input class="form-control" id="amountValue" type="number" min="50" max="<?php if($amount3) {echo $amount3; } else {  echo $amount1; }?>" placeholder="Minimum 50 - Maximum <?php if($amount3) {echo $amount3; } else {  echo $amount1; }?>" name="amount" value="<?php echo $amount;?>" required>
...
?>
  ...
  <div class="row">
    ...
    <div class="col-md-8">
      ...
      <input type="text" class="form-control" id="spBankuser" name="spBankusername" value="<?php echo $spBankusername;?>" required>
  ...
  <input type="text" class="form-control" id="spBankname" name="spBankname" value="<?php echo $spBankname;?>" required>
  ...
  <input type="text" class="form-control" id="spBanknumber" name="spBanknumber" value="<?php echo $spBanknumber;?>" required>
  ...
  <input type="text" class="form-control" id="spBranchnumber" name="spBranchnumber" value="<?php echo $spBranchnumber;?>" required>
  ...
  <input type="text" class="form-control" maxlength="18" id="spAccountnumber" name="spAccountnumber" value="<?php echo $spAccountnumber;?>" required>
  ...
  <div class="col-md-8">
    ...
    <input type="text" class="form-control" maxlength="11" id="spBankcode" name="spBankcode" value="<?php echo $spBankcode;?>" required>
...
<button type="submit" name="submitBtn" id="submitBtn" class="btn btn-primary btn-border-radius" onClick="return false;" >Save changes</button>
</div>
...
<form action="" method="post" id="smsEmailForm" style="display: none;" onSubmit="return false;">
  <div class="row">
    <div class="col-md-2"></div>
      <div class="col-md-8">
        <div class="form-group">
          <label for="smsField">Phone sms otp<span class="red">*</span></label>
          <input type="text" class="form-control" id="smsField" name="smsField" required />
        </div>
      </div>
  </div>
  <div class="row">
    <div class="col-md-2"></div>
      <div class="col-md-8">
        <div class="form-group">
          <label for="emailField">Email verification code<span class="red">*</span></label>
          <input type="text" class="form-control" id="emailField" name="emailField" required />
        </div>
      </div>
  </div>
<div class="modal-footer">
<button type="submit" id="submitSmsEmailBtn" name="submitSmsEmail" class="btn btn-primary btn-border-radius">Submit</button>
  </div>
</form>
...
$(document).ready(function() {
  ...
  $("#submitBtn").click(function(e) {
    ...
    var enteredAmount = parseInt($("input[name='amount']").val());
    if (enteredAmount < 50) {
      alert("Amount should between 50 to 5000");
      document.getElementById('loader').style.display = 'none';
      return;
    }
    if (!withdrawalForm.checkValidity()) {
      document.getElementById('loader').style.display = 'none';
      if(!document.getElementById("amountValue").checkValidity()){
        alert(document.getElementById("amountValue").validationMessage);
        return;
      }
      if(!document.getElementById("spAccountnumber").checkValidity()){
        alert(document.getElementById("spAccountnumber").validationMessage);
        return;
      }
      if(!document.getElementById("spBankcode").checkValidity()){
        alert(document.getElementById("spBankcode").validationMessage);
        return;
      }
    ...
    if(enteredAmount > <?php echo $amount3 ?>){
      alert("You don't have enough money in the wallet");
      document.getElementById('loader').style.display = 'none';
      return;
    }
    $.ajax({
  ...
  $("#submitSmsEmailBtn").on('click', function () {
      var smsOTP = $("#smsField").val();
      var emailOTP = $("#emailField").val();
      $.ajax({
        type: "POST",
        url: "withdrawalcodecompare.php",
        data: { smsOTP: smsOTP, emailOTP: emailOTP },
        dataType: 'json',
        success: function (response) { 
          if (response.status == 0){
            alert(response.message);            
