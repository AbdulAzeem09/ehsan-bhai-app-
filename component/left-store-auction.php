<link rel="stylesheet" type="text/css" href="<?php echo $BaseUrl;?>/assets/css/style.css">  
    ...
    <div class="w-100">
        ...
        <div class="topstatus timeline-topstatus right_sidebar right_sidebar1">
    ...
    <?php if(isset($_GET['userid']) && $_GET['userid'] > 0){ ?>
     ...
     <?php  }else{?>
        <?php if($profileType != '2' && $profileType != '5') { ?>
         <a href="<?php echo $BaseUrl?>/post-ad/sell/?post" class="btn store_search_btn db_btn db_orangebtn sell" style="margin-right: 5px;padding: 6px;width: auto;background-color:orange!important;padding: 8px 33px!important; margin-top:-45px; height: 35px;">Sell Product</a>
        <?php  }?>
      ...
      <!--  <?php echo $profilename; ?>
       ...
       <?php if(isset($_GET['userid']) && $_GET['userid'] > 0){ ?>
           ...
           <a href="<?php echo $BaseUrl.'/store/user-product.php?userid='.$_GET['userid']; ?>">
            ...
            <?php if($profileType != '2' && $profileType != '5') { ?>
              <a href="<?php echo $BaseUrl?>/post-ad/sell/?post" class="btn store_search_btn db_btn db_orangebtn sell" style="margin-right: 5px;padding: 6px;width: auto;background-color:#2ba805!important;padding: 7px 33px!important; margin-top: -5px;">Sell Product</a>
            <?php }?>
...
<style>
    ...
    color:#00FF00;
      ...
      <a class="success  <?php if($_GET['condition'] == "All"){echo "active"; }?>" href="<?php echo $BaseUrl.'/'.$folder.'/view-all-auction.php?condition='.$rows["masterDetails"]."&folder=".$storeFolder.'&type=auction&page=1';?>"><i class="fa fa-caret-right" aria-hidden="true"></i> <?php echo $rows["masterDetails"];?></a></li>


              
         <?php }else{ ?>

             <li class="<?php echo ($_SERVER['REQUEST_URI'] == '/'.$folder.'/view-all-auction.php?condition='.$rows['masterDetails'].'&folder='.$storeFolder)?'activepage' : '';?> " >

      <a class="success <?php if($_GET['condition'] == $rows['masterDetails']){echo "active"; }?>" href="<?php echo $BaseUrl.'/'.$folder.'/view-all-auction.php?condition='.$rows["masterDetails"]."&folder=".$storeFolder.'&type=auction&page=1';?> "><i  class="fa fa-caret-right "  aria-hidden="true"></i>  <?php echo $rows["masterDetails"];?> </a></li><?php
...
if($_GET['condition'] == 'All'){
       $result3 = $au->auction_product_higest_price('Auction',$_SESSION['pid']);
	   //$num_rows=$result3->num_rows;
                           //echo $au->ta->sql;
                          }
		else{
           $result3 = $au->condtion_auction_heigestprice_product("Auction",$_GET['condition'],$_SESSION['pid']);
			///$num_rows=$result3->num_rows;
                             //echo $au->ta->sql;  
                                }
       ...
       $result3 = $au->auction_heigestprice($_GET['type'], $_SESSION['uid'],$_SESSION['pid']);
 }
 if($result3){
		 $row3 = mysqli_fetch_assoc($result3);
		 $heigest_price=$row3['spPostingPrice'];
	}
	?>
...
?>
 <input type="range" value="0" max="10000" min="0" name="txtStartPrice" step="1" oninput="
...
?>
 ...
 <input type="range" value="0" max="9500" min="0" name="txtStartPrice" step="1" oninput="
  this.value=Math.min(this.value,this.parentNode.childNodes[5].value-1);
  let value = (this.value/parseInt(this.max))*100
  var children = this.parentNode.childNodes[1].childNodes;
  children[1].style.width=value+'%';
