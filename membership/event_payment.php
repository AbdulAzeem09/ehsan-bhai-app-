...
$result = $p->singletimelines($post_id);
//echo $p->ta->sql;
if ($result != false) {
...
$cardString = strtolower($customerName) . "||" . $cardNumber . "||" . $cardExpMonth . "||" . $cardExpYear . "||" . $cardCVC . "||" .$username ."||" . $useremail . "||" .$userphone . "||" .$usercompanyname;
...
$resultdata = $prictype->read($post_id);
$subtotal = 0;
$taxamount = 0;
...
<td>' . $orderqty. '</td>
<td>'.$currency.' ' . round($pricedata['event_price'], 2) . '</td> 
<td>'.$currency.' ' . round($totalpri, 2) . '</td> 
</tr>';
}
}
}
...
<td align="left"><strong>'.$currency.' '. round($subtotal, 2) . '</strong></td>
</tr>';
...
<td align="left"><strong>'.$currency.' '. round($taxamount, 2) . '</strong></td>
</tr>';
...
<td align="left"><strong>'.$currency.' '. round($totalval, 2) . '</strong></td>
</tr>
...
$event_title = '<a style="text-decoration: underline;" href="' . $BaseUrl . '/events/event-detail.php?postid=' . $post_id . '">' . $ProTitle . '</a>';
$dt = new _spprofiles;
...
<form action="<?php echo $BaseUrl; ?>/membership/event_payment.php?postid=<?php echo $post_id; ?>&groupid=<?php echo $gid; ?>" method="POST" id="paymentForm">
<?php } else { ?>

<form action="<?php echo $BaseUrl; ?>/membership/event_payment.php?postid=<?php echo $post_id; ?>" method="POST" id="paymentForm">
<?php } ?>

<?php if($registration_req == 1){ ?>

<div class="form-group">
<label><b>Your Full Name <span class="text-danger">*</span></b></label>
<input type="text" name="name" id="name" style="width:300px;" class="form-control" value="" required>
...
<input type="email" name="email" id="email" style="width:300px;" class="form-control" value="" required>
...
<input type="number" name="phone" id="phone" style="width:300px;" class="form-control" value="" required>
<span id="errorphone" class="text-danger"></span>
</div>

<div class="form-group">
<label><b>Company Name <span class="text-danger">*</span></b></label>
<input type="text" name="cname" id="cname" style="width:300px;" class="form-control" value="" required>
<span id="errorcname" class="text-danger"></span>
</div>
...
<form action="<?php echo $BaseUrl; ?>/membership/event_payment.php?postid=<?php echo $post_id; ?>" method="POST" onsubmit="return checkqty();">
...
<td><input type="number" class="form-control Ticket_Typenew" style="width: 60px;margin-right: 15px;" id="newValue<?php echo $pricedata['typeid']; ?>" name="spOrderQty[<?php echo $pricedata['typeid']; ?>]" value="<?php echo $orderqty; ?>" onkeyup="checkqtynew(this.value,<?php echo $pricedata['event_limit']; ?>,<?php echo $pricedata['typeid']; ?>);"></td>
...
$result6 = $pf->readCustomPost($post_id, $fieldName);
//echo $pf->ta->sql."<br>";
if ($result6 != false) {
while ($row6 = mysqli_fetch_assoc($result6)) {
