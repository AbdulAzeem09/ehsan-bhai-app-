  ...
  if($coupon_code && $package){
    ...
    if($discountObj){    
      ...
      if(strtotime($discountObj['expiry_date']) < strtotime($today)){
        $paymentMessage = "<strong>Coupon code expired</strong>";
      }
      else{
        $deduct_amount = ($discountObj['percentage']*$totalAmount)/100;
        $totalAmount = $totalAmount-$deduct_amount; //discount amount;    
      }
    }
    else{
      $paymentMessage = "<strong>Coupon invalid</strong>";
    }
  }
...
//}
    ...
    $tier1_user = $m->getReferrelUserByCode($refferalcodeused['refferalcodeused']);
    $tier1_userid = mysqli_fetch_assoc($tier1_user);

    $tier2_user = $m->getReferrelUserByCode($tier1_userid['refferalcodeused']);
    $tier2_userid = mysqli_fetch_assoc($tier2_user);


    $tier3_user = $m->getReferrelUserByCode($tier2_userid['refferalcodeused']);
    ...
    if($tier2_userid){
        if($tier2_userid['role']){
            if($tier2_userid['role'] == "super vip"){
                $tier1_commission =($_POST['total_amount']*$commission_type['super_vip_sub_commission'])/100;
            }
            if($tier2_userid['role'] == "vip"){
                $tier1_commission =($_POST['total_amount']*$commission_type['vip_sub_commission'])/100;
            }
        }else{
            $tier1_commission =($_POST['total_amount']*$commission_type['general_sub_commission'])/100;
        }
    }else{
        $tier1_userid = '';
        $tier1_commission = '';
    ...
    if($tier2_userid){
        $tier2_commission = ($_POST['total_amount']*5)/100;
    }else{
        $tier2_userid = '';
        $tier2_commission = '';
    }
    if($tier3_userid){
        $tier3_commission=($_POST['total_amount']*5)/100;
    }else{
        $tier3_userid = '';
        $tier3_commission = '';
    }
    ...
    $data = [
        ...
        'tier1_userid'=>$tier1_userid['idspUser'],
...
$super_commission = mysqli_fetch_assoc($ss); 
$total_comm=($_POST['price']*$super_commission['super_vip_sub_commission'])/100;
...
$super_commission = mysqli_fetch_assoc($ss); 
$total_comm=($_POST['price']*$super_commission['vip_sub_commission'])/100;
...
$total_comm=($_POST['price']*$super_commission['general_sub_commission'])/100;
      ...
      <div class="col-md-12" style="padding-left:0px;">
        <div class="row">
          <div class="col-md-4" style="padding-left:0px;">
            <input class="form-control" type="text" name="coupon_code" id="couponCode" />                              
          </div>
          <div class="col-md-3">
            <button type="button" onClick="validateCoupon();return false;" class="btn butn_cancel  btn-border-radius" >Validate</button>
          </div>
          
          <div class="col-md-5 text-danger" id="errorCoupon">
          
          </div>
          
        </div>
      </div>
      <div class="col-md-12 plan-chrg" id="couponPlan" style="display:none;color:green;font-weight:bold;" >
...
<form action="<?php echo $BaseUrl;?>/membership/member_buy.php?id=<?php echo $id;?>" method="POST" id="paymentForm">	
...
<button type="submit" class="btn butn_cancel  btn-border-radius" name="payNow" id="payNow"  style="" onclick="stripePay(event)" value="Pay Now"><span class="glyphicon glyphicon-shopping-cart" aria-hidden="true"></span>Pay Now</button>
...
function validateCoupon(){
  ...
  if(cc !== ""){
    ...
    if(cc){
      $.ajax({
        url: "/membership/checkDiscount.php", type: "POST",
        data: {coupon_code: cc, package_id_check:<?php echo $id;?>}, dataType: "json",
        success: function(result){
          if(result.error !== undefined && result.error !== ""){
            $("#couponPlan").hide();
            $("#errorCoupon").html(result.error);
          } 
          else{
            $("#errorCoupon").html();
            $("#couponPlan").show();
            $("#couponPlan").html(result.discountText);
          }
        }
      });
    }
    else{
      $("#errorCoupon").html("Please provide the Coupon code");  
    }
  }
  else{
    $("#errorCoupon").html("Please provide the Coupon code");
