...
while($rows=mysqli_fetch_assoc($res)) {   
 ...
 $profile_id =  $rows['spProfiles_idspProfiles']; 
...
<a style="font-size:16px" href="' . $baseurl . '/friends?profileid=' . $rows['spProfiles_idspProfiles'] . '">' . $row2['spProfileName'] . '<br> (' . $profile_name . ')</a><br><br></div>';

} else {
	echo "<div class='col-md-3' style='margin-right:10px'>";
  echo "<img src='" . $BaseUrl . "/img/default-profile.png' height='50px' width='50px' style='border-radius:50px; margin-right:33px'>" . "
  </div>
<div class='col-md-9' style='text-align:left; margin-left:55px; margin-top:-54px;font-size: 18px;'>
<a style='font-size:16px' href='" . $baseurl . "/friends?profileid=" . $rows['spProfiles_idspProfiles'] . "'>" . $row2['spProfileName'] . "<br> (".$profile_name.")</a></div>";
