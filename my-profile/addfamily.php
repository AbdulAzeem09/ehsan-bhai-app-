	...
	if (!mysqli_query($con, $sql)) {
		...
		if ($id = mysqli_query($con, $sqll)) {
			while ($row = mysqli_fetch_assoc($id)) {
				...
				echo "<tr class='deleterecord" . $row['id'] . "'><td>" . $numrows . "</td><td>" . $row['membername'] . "</td><td>" . $row['memberrelation'] . "</td><td><a href='#' id='del_fam' onclick='delfam(" . $row['id'] . ");'><i class='fa fa-trash' aria-hidden='true' ></i></a>&nbsp;<a href='#'  data-toggle='modal' data-target='#editfamilymember" . $row['id'] . "'><i class='fa fa-pencil' aria-hidden='true'></i></a></td></tr>";
			}
		}
	}
}
