...
if($res != false){
  ...
  die("Logged out. Please re-login");
}
?>
...
<body class="bg_gray">
			...
			<div class="modal-content sharestorepos bradius-15" >
				...
				<form id="uploadidentityfrm" enctype="multipart/form-data" >
					...
					<div class="modal-body">
						  ...
						  ?>
							...
							<div class="form-group">
								...
								<?php if (!empty($row['idimage'])) { ?>
									...
									left: 16px!important;">Date of Upload : <?php echo date('d/m/Y', $row['created_on']);?></h5>
						...
						class="btn btn-submit db_btn db_primarybtn btn-border-radius">Keep this file</button>
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content sharestorepos bradius-15">
				<form action="../authentication/change.php" method="post" class="">
					...
					<div class="modal-body">
						<div class="form-group">
							<label for="oldpassword" class="control-label contact">Old Password <span class="red">*</span></label><span class="red" id="oldpwd"></span>
							<input type="password" class="form-control" onkeyup="clrold()" id="oldpassword" name="oldpassword_">
						</div>

						<div class="form-group">
							<label for="newpassword" class="control-label contact">New Password <span class="red">*</span></label><span class="red" id="npass"></span>
							<input type="password" class="form-control" onkeyup="clrnew()" id="newpassword" name="spUserPassword">
						</div>

						<div class="form-group">
							<label for="typenewpassword" class="control-label contact">Confirm New Password <span class="red">*</span></label><span class="red" id="cnpass"></span>
							<input type="password" class="form-control" onkeyup="clrcnew()" id="typenewpassword" name="spUserPassword_">
						</div>
					</div>
					<div class="modal-footer bg-white br_radius_bottom">
						<button type="button" class="btn butn_cancel btn-close db_btn db_orangebtn" data-dismiss="modal">Close</button>
						<button type="submit" id="changepassword" style="color:red" class="btn btn-submit btn-border-radius db_btn db_primarybtn">Change</button>
					</div>
				</form>
			</div>
		</div>
	...
	<script>
		...
		function clrnew(){
			document.getElementById("npass").innerText="";
		}
		function clrcnew(){
			document.getElementById("cnpass").innerText="";
		}
	</script>
...
<div class="col-md-9 bg_white" style="padding-bottom: 15px; margin-top: 10px;">
	...
	<div class="col-md-8">
		...
		<h3 class="add_shippinglabel">Edit your address</h3>

<?php  
$id = !empty($_GET["id"]) ? $_GET["id"] : "";
$row1 = selectQ("SELECT * from addshipping_address where id=?", "i", [$id], "one");
...
<form id="address-ui-address-form" action="">

	<input type="hidden" name="shipping_id" value="<?php echo $row1["id"]; ?>">


	<div class="form-group">
		<label class="add_shippinglabel" for="shipp_username">Name:</label>

		<input type="text" class="form-control" placeholder="Enter Your Name" id="shipp_username" name="fullname" value="<?php echo $row1["fullname"]; ?>">
	...
	<div class="form-group">
		<label class="add_shippinglabel" for="shipp_address">Address:</label>
		<textarea class="form-control" rows="2" name="address" id="shipp_address" value="<?php echo $row1["fulladdress"]; ?>"><?php echo $row1["fulladdress"]; ?></textarea>
		<span id="shippaddress_error" style="color:red;"></span>
	</div>

<div class="form-group">
	<label for="spProfilesCountry">Country:</label>
	<select id="spProfilesCountry" class="form-control " name="country" value="<?php echo $row1["country"]; ?>">
        ...
        if(!empty($row1['country'])){
          if($row1['country'] == $row3['country_id']){
            $selected = "selected";
          }
        }
        else if($ruser['spUserCountry'] == $row3['country_id']){
          $selected = "selected";
        }
      
				?>
				<option value='<?php echo $row3['country_id'];?>' <?php echo $selected; ?>>
	...
	<!--  <div class="loadState"> -->
		<!--  <?php print_r($row1['state']);?> -->
		<div class="form-group">
			<label for="spPostState_">State:</label>

			<select class="form-control spPostField" id="spPostState_" name="spUserState" value="<?php echo $row1["state"]; ?>">
...
// print_r($countryId);
				...
				if($result2 != false){
					while ($row2 = mysqli_fetch_assoc($result2)) { 
						...
						<option value='<?php echo $row2["state_id"];?>'
							...
							<?php echo ($row1['state'] == $row2["state_id"] )?'selected':'';?> ><?php echo $row2["state_title"];?> </option>
							<?php
						}
					}
			...
			<div class="form-group">
				...
				<select class="form-control spPostField" id="shipp_city" name="city" value="<?php echo $row1["city"]; ?>">
					...
					if($result3 != false){
						while ($row3 = mysqli_fetch_assoc($result3)) { 
							...
							<option value='<?php echo $row3["city_id"];?>'
								...
								<?php echo ($row1['city'] == $row3["city_id"] )?'selected':'';?> ><?php echo $row3["city_title"];?> </option>
								<?php
							}
						}
...
<div class="form-group">
	...
	<input type="text" class="form-control" id="shipp_zipcode" placeholder="6 digits [0-9] zipcode" name="zipcode" value="<?php echo $row1["zipcode"]; ?>">
	<span id="shippcity_error" style="color:red;"></span>
</div>

<div class="form-group">
	<label class="add_shippinglabel" for="respUserEphone">Phone number:</label>

	<input type="text" class="form-control"  id="respUserEphone"  placeholder="10 digit mobile number" name="phone" value="<?php echo $row1["phone"]; ?>">
	<span id="shipplandmark_error" style="color:red;"></span>

</div>

<div class="form-group">
	<label class="add_shippinglabel" for="shipp_landmark">Land Mark:</label>

	<input type="text" class="form-control" id="shipp_landmark" placeholder="E.g. Near AIIMS Flyover, Behind Regal Cinema, etc." name="landmark" value="<?php echo $row1["landmark"]; ?>">
...
<div class="form-group">
	...
	<select class="form-control form-control-lg" name="address_type" value="<?php echo $row1["address_type"]; ?>">


		<option>Select an Address Type</option>
		<option  value="Residential" <?php if ( $row1['address_type'] == "Residential" ) { echo "selected"; } ?>>Residential </option>
		<option  value="Business" <?php if ( $row1['address_type'] == "Business" ) { echo "selected"; } ?>>Business </option>
		<option value="Postal Box" <?php if ( $row1['address_type'] == "Postal Box" ) { echo "selected"; } ?>>Postal Box</option>
		<option  value="Store" <?php if ( $row1['address_type'] == "Postal Box" ) { echo "selected"; } ?>>Store </option>
		<option  value="Warehouse" <?php if ( $row1['address_type'] == "Warehouse" ) { echo "selected"; } ?>>Warehouse </option>
...
<div class="form-group">
	...
	<textarea class="form-control" rows="2" name="delivery_instructions" value="<?php echo $row1["delivery_Instructions"]; ?>"><?php echo $row1["delivery_Instructions"]; ?></textarea>
</div>
...
<div class="form-group">        
	<div class="">
		<button type="submit" class="btn btn-default Add_adderess btn-border-radius" >Save Change</button>
	</div>
</div>
...
<script type="text/javascript">
	...
	$('#respUserEphone').keypress(function(){
//alert('1');
		var phone = $('#respUserEphone').val();
...
// var shipadd= $("#shipping_address").val()
			...
			var shipcountry = $("#spProfilesCountry").val()
			var shipstate = $("#spPostState_").val()
			...
			if(shipusername == "" &&  shipaddress == "" && shipcountry.length == 0 && shipstate == 0 && shipcity == "" &&  shipzipcode == "" && shipphone == "" && shiplandmark == ""){
				...
				$("#shipplandmark_error").text("Please Enter Your Landmark Address.");
				$("#shipp_landmark").focus();

				return false;

			}else if (shipusername == "") {
...
*/
				...
				$.ajax({
					type: 'POST',
					url: '../authentication/editshipping_address.php',
					data: new FormData(this),
					processData: false,
					contentType: false,
...
//console.log(data);
						...
						function() {
							...
							window.location = "<?php echo $BaseUrl;?>/my-profile/add-shipping.php";
