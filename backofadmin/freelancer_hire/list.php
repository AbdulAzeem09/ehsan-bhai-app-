<?php
	...
	$rowsPerPage = 25;
  	$sql		=	"SELECT * FROM freelance_project  ORDER BY id DESC";
...
?>
	...
	<section class="content-header ">
		<h1>Freelancer<small>[List]</small></h1>
	</section>
	<!-- Main content -->
							...
							if ($result){
								...
								while($row = dbFetchAssoc($result)) {
									...
									<tr>
										...
										<td><a href="<?php echo $BaseUrl.'freelancer/dashboard/freelance_project_detail.php?postd='.$row["id"];?>;'">Project for <?php echo showProfileName($dbConn, $receiver_idspProfiles);	?></a></td>
										<td class="text-center"><?php echo date("d-M-Y", $postDate); ?></td>
										<td><?php echo showAcountNameProfile($dbConn, $sender_idspProfiles); ?></td>

										
										<td><?php showProfileName($dbConn, $receiver_idspProfiles); ?></td>
                                            <!----------dumy-------->
											<td><?php if($row['status']==0){ echo'Pending';} elseif($row['status']==1){ echo 'Accepted';} else{ echo 'Rejected';}?></td>
