...
<body>
	...
	<?php 
					...
					$sa = new _businessrating;
					$sal = $sa->read_id_business($_GET['postid']);
					//print_r($sal);
					$i=1;
					if($sal!=false){
					$row=mysqli_fetch_assoc($sal);
	...
	<div class="row" style="background-color: white;height: 2px;">
		...
		<div class="container" style="background: linear-gradient(to bottom, #7dba41 0%, #468e4f 100%);" data-color-opacity="0.72">
         ...
         if($_GET['draft']==1){
				?>
				<form action="updatebusiness.php?draft=1&postid=<?php echo $_GET['postid'];?>" method="post" novalidate enctype="multipart/form-data" id="paymentForm">
		<?php } else { ?>
				<form action="updatebusiness.php?postid=<?php echo $_GET['postid'];?>" method="post" novalidate enctype="multipart/form-data" id="paymentForm">
			<?php } ?>		
				...
				<section>
					...
					<select class="select-single-item" name="Type" id="type">
						...
						<option value="1" name="franchise" <?php if($row['business_type']==1){echo 'selected';}?> >Franchise</option>
						<option value="2" name="sale" <?php if($row['business_type']==2){echo 'selected';}?>>Independent sale</option>
					</select>
				...
				<section>
					<p>Business Detail</p>
						...
						<select class="select-single-item" name="Status" >
						  ...
						  <option value="1" name="for_sale" <?php if($row['business_status']==1){echo 'selected';}?>>For sale</option>
						  <option value="2" name="under_offer" <?php if($row['business_status']==2){echo 'selected';}?>>under offer</option>
						  <option value="3" name="sold" <?php if($row['business_status']==3){echo 'selected';}?>>sold</option>
						</select>-->
						...
						<select class="select-single-item" name="category" id="cat">
							...
							<option value="1" name="manufacturing" <?php if($row['business_category']==1){echo 'selected';}?> >Manufacturing</option>
							<option value="2" name="hotel" <?php if($row['business_category']==2){echo 'selected';}?>>Hotel</option>
							<option value="3" name="website" <?php if($row['business_category']==3){echo 'selected';}?>>Website Design</option>
						</select>
				...
				<section>
					<p>Real Estate Included? <span style="color:red" id="rs">*</span></p>
						...
						<div class="row" style="text-align: left;">
							<div class="checkboxes">
								<input id="check-2" type="radio" value="1" name="real_estate" <?php if($row['real_state_included']==1){echo 'checked';}?> >
								<label for="check-2" >Yes</label>
								<input id="check-3" type="radio" value="0" name="real_estate" <?php if($row['real_state_included']==0){echo 'checked';}?> >
						...
						<div class="row" style="text-align: left;">
							<div class="checkboxes margin-bottom-20">
								<input id="check-2" type="radio" value="1" name="inventory" <?php if($row['inventory_includes']==1){echo 'checked';}?>>
								<label for="check-2">Per Annum if applicable</label>
								<input id="check-3" type="radio"  value="0" name="inventory" <?php if($row['inventory_includes']==0){echo 'checked';}?>>
						...
						<div class="row" style="text-align: left;">
							<div class="checkboxes in-row margin-bottom-20">
								<input id="check-2" type="radio"  value="1" name="furniture_fixture" <?php if($row['includes_furnitures']==1){echo 'checked';}?>>
								<label for="check-2">Furniture</label>
								<input id="check-3" type="radio"  value="0" name="furniture_fixture" <?php if($row['includes_furnitures']==0){echo 'checked';}?>>
						...
						<div class="row" style="text-align: left;">
							<div class="checkboxes margin-bottom-20">
								<input id="check-2"  type="radio" value="1" name="sale_software" <?php if($row['sale_software']==1){echo 'checked';}?>>
								<label for="check-2">Review listing</label>
								<input id="check-3" type="radio" value="0" name="sale_software" <?php if($row['sale_software']==0){echo 'checked';}?>>
					...
					<h5 style="text-align: left;">Training Support Provided   <span style="color:red" id="tr_support">*</span></h5>
								<div class="row" style="text-align: left;">
									<div class="checkboxes margin-bottom-20">
										<input id="check-5" type="radio" value="1" name="tr_support" <?php if($row['training_support']==1){echo 'checked';}?>>
										<label for="check-2">Yes</label>
										<input id="check-5" type="radio" value="0" name="tr_support" <?php if($row['training_support']==0){echo 'checked';}?>>
										<label for="check-3">No</label>
									</div>
								</div>
								...
								<div id="dvPreview" >
									<?php
										...
										if(isset($_GET['postid'])){
											$res = $pic->read_files($_GET["postid"]);
											//var_dump($res);
											if ($res != false) {
												while ($rows = mysqli_fetch_assoc($res)) {
							 						$picture = $rows['filename'];
													//echo $picture;
echo "<div class='col-md-4 imagepost'><span class='fa fa-remove img_file dynamicimg closed' style='margin-right:10px;'   data-work='event' data-aws='6' data-src='" . $rows['filename'] . "'  data-pic='".$rows['id']."'></span><img class='overlayImage' style='width:100%; height: 80px; margin-right:5px;' data-name='fi_".$i."' src='" .$BaseUrl .'/business_for_sale/uploads/'.($picture) . "'/> <a href='".$BaseUrl .'/business_for_sale/uploads/'.($picture) . "' download> Downlaod</a></br><label style='font-size: 10px;' class='updateFeature' data-postid='".$_GET['postid']."' data-picid='".$rows['id']."'></div>";
							$i++;
  ...
  success: function(html){
					...
					$sup = $sa->read_support_files($_GET['postid']);
					//print_r($sup);
							...
							<div class="row">
								<div id="dvPreview" >
									<?php
										...
										if(isset($_GET['postid'])){
											$res1 = $pic1->read_support_files($_GET["postid"]);
											//var_dump($res1);
											if ($res1 != false) {
												...
												//print_r($rows1);
													...
													//echo $picture1;
echo "<div class='col-md-4 imagepost' id='support_".($rows1['id']) ."'><span class='fa fa-remove support dynamicimg closed' style='margin-right:10px;'   data-work='event' data-aws='6' data-src='" . $rows1['filename'] . "'  data-pic1='".$rows1['id']."'></span><img class='overlayImage' style='width:100%; height: 80px; margin-right:5px;' data-name='fi_".$i."' src='" .$BaseUrl .'/business_for_sale/uploads/'.($picture1) . "'/></br><a href='".$BaseUrl .'/business_for_sale/uploads/'.($picture1) . "' download> Downlaod</a><label style='font-size: 10px;' class='updateFeature' data-postid='".$_GET['postid']."' data-picid='".$rows1['id']."'></div>";
							$i++;
...
<script src="https://rawgit.com/enyo/dropzone/master/dist/dropzone.js"></script>
				...
				<div class="row" style="text-align: center;">
					<div class="button zoom1" id="cancel"><a href="dashboard/draft.php" style="color:white;">Cancel</a></div>
					<div class="" id="update"><a type="submit" name="submit"  id="update" class="button zoom1">Update</a></div>
					
					<!--<a type="submit" name="submit" value="Update" id="update" class="button zoom1">-->
					<?php if($_GET['draft']==1){ ?>
