...
<?php
	...
	function add($dbConn){
		...
		if (dbNumRows($result) > 0) {
			...
			$sql2 = "INSERT INTO tbl_page(page_title, page_content, status) VALUES ( '$txtTitle', '".$txtDesc."', '$radStatus')";
	...
	function modify($dbConn){
		...
		$result = dbQuery($dbConn,$sql);
		if ($result) {
			$sql2 = "UPDATE tbl_page SET page_content = '$txtDesc', status = '$radStatus' WHERE page_id = $hidId";
		}else{
			$sql2 = "UPDATE tbl_page SET page_title = '$txtTitle', page_content = '$txtDesc', status = '$radStatus' WHERE page_id = $hidId";
		}
		//echo $sql2;
