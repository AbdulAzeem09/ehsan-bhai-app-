<?php
	...
	$rowsPerPage = 25;
  	$sql		=	"SELECT * FROM sptrainig_wallet";
  ...
  if($sdate && $edate && $selectname){
   $query2="SELECT * FROM sptrainig_wallet WHERE seller_userid='$selectname' AND date_txn BETWEEN '$sdate' AND '$edate'";
  }
 else if(($sdate && $edate)&& !($selectname)){
  $query2="SELECT * FROM sptrainig_wallet WHERE  date_txn BETWEEN '$sdate' AND '$edate'";
  }
  else if(($selectname)&& !($sdate && $edate)){
  
  $query2="SELECT * FROM sptrainig_wallet WHERE seller_userid='$selectname'";
  }
...
?>
	...
	<!-- Content Header (Page header) -->
		...
		<h1> Training Wallet<small>[List]</small></h1>
		...
		<a href="<?php  $_SERVER["DOCUMENT_ROOT"] ?>/backofadmin/trainingwallet/index.php?view=traininglist" class="btn btn-primary">Reset</a>
	</section>
	<!-- Main content -->
