<?php
 ...
 $query2="SELECT * FROM spbonuswallet WHERE uid='$selectname' AND date_txn BETWEEN '$sdate' AND '$edate'";
  }
  else if(($sdate && $edate)&& !($selectname)){
  $query2="SELECT * FROM spbonuswallet WHERE date_txn BETWEEN '$sdate' AND '$edate'";
  }
   else if(($selectname)&& !($sdate && $edate)){
  $query2="SELECT * FROM spbonuswallet WHERE uid='$selectname' ";
  }
...
?>
	...
	<section class="content-header">
		<h1>Wallet Store<small>[List]</small></h1>
		...
		<a href="<?php  $_SERVER["DOCUMENT_ROOT"] ?>/backofadmin/Walletbonus/index.php?view=bonuslist" class="btn btn-primary">Reset</a>
	</section>
	<!-- Main content -->
