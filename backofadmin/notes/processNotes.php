...
<?php
	...
	function modify($dbConn){
		...
		$txtTaskMenu	= mysqli_real_escape_string($dbConn, $_POST['txtTaskMenu']);
		$txtShrtDesc	= mysqli_real_escape_string($dbConn, $_POST['txtShrtDesc']);
		$txtDesc		= @filterWords($_POST['txtDesc']);
		//$txtDesc 		= $_POST['txtDesc'];
		
		$sql = "UPDATE tbl_notes SET spNotesTitle = '$txtTitle', spNotesDesc = '$txtDesc', user_id_to = '$user_id_to', spNoteMenu = '$txtTaskMenu', spNoteShrtDesc = '$txtShrtDesc' WHERE idspNotes = $hidId";		
		$result = dbQuery($dbConn, $sql);
		$_SESSION['count'] = 0;
		$_SESSION['data'] = "success";
		$_SESSION['errorMessage'] = "Updated Successfully.";
	...
	function addd($dbConn){
		...
		$sql2 = "INSERT INTO tbl_notes(spNotesTitle, spNotesDesc, spNotesStatus, user_id_from, user_id_to, spNoteMenu, spNoteShrtDesc) VALUES
		 ( '$txtTitle', '$txtDesc', '$spNotesStatus', '$user_id_from', '$user_id_to', '$txtTaskMenu', '$txtShrtDesc')";
	...
	function deletee($dbConn) {
		...
		$sql = "UPDATE tbl_notes SET spNotesStatus = '-7' WHERE idspNotes = $noteId ";
