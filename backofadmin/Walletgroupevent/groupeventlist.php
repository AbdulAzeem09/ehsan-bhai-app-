<?php
	...
	$rowsPerPage = 25;
  	$sql		=	"SELECT * FROM spgroup_event_wallet";
  ...
  if($sdate && $edate && $selectname){
   $query2="SELECT * FROM spgroup_event_wallet WHERE buyer_userid='$selectname' AND date_txn BETWEEN '$sdate' AND '$edate'";
  }
  else if(($sdate && $edate)&& !($selectname)){
   $query2="SELECT * FROM spgroup_event_wallet WHERE date_txn BETWEEN '$sdate' AND '$edate'";
  
  }
   else if(($selectname)&& !($sdate && $edate)){
   $query2="SELECT * FROM spgroup_event_wallet WHERE buyer_userid='$selectname' ";
  }
...
?>
	...
	<!-- Content Header (Page header) -->
		...
		<h1> Group Event <small>[List]</small></h1>
		...
		<a href="<?php  $_SERVER["DOCUMENT_ROOT"] ?>/backofadmin/Walletgroupevent/index.php?view=groupeventlist" class="btn btn-primary">Reset</a>
	</section>
	<!-- Main content -->
