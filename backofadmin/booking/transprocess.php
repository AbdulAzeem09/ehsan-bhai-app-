...
//die('000');
	...
	function addremark($dbConn){
		...
		if(dbNumRows($result2) > 0){
			...
			// Insert
			$sql = "UPDATE spevent_transection SET remark ='$remark' WHERE id = $trans_id";
			$result = dbQuery($dbConn, $sql);
			//$result = dbQuery($dbConn, $sql);
			$_SESSION['count'] = 0;
			$_SESSION['errorMessage'] = "Remark Added Successfully.";
			$_SESSION['data'] = "success";
			header("Location: https://thesharepage.com/backofadmin/booking/index.php?view=event");
			//redirect('https://dev.thesharepage.com/backofadmin/booking/index.php?view=event');	
		/*}*/
	}
