...
<?php
	...
	function modify($dbConn){
		...
		$sql2 = "SELECT idartsize FROM artsizes WHERE spSizeTitle = '$txtTitle'";
		...
		if(dbNumRows($result2) > 0){
			...
			// Insert
			$sql = "UPDATE artsizes SET spSizeTitle = '$txtTitle' WHERE idartsize = $hidId";
	...
	function add($dbConn){
		...
		if(dbNumRows($result2) > 0){
			...
			// Insert
			$sql   = "INSERT INTO artsizes (spSizeTitle) VALUES ('$txtTitle')";
			$result = dbQuery($dbConn, $sql);
			$_SESSION['count'] = 0;
	...
	function deletee($dbConn){
		...
		$sql		=	"DELETE FROM artsizes WHERE idartsize = $sizeId";
		$result 	= 	dbQuery($dbConn, $sql);
		$_SESSION['count'] = 0;
