...
<?php
	...
	function add($dbConn){
		...
		$sql2 =	insertQ("insert into sptestimonial(name, designation , image , description ) values(?, ?, ?, ?)", "ssss", [$name, $designation, $urll, $txtDesc]);
		$_SESSION['count'] = 0;
	...
	function modify($dbConn){
		...
		if (isset($_FILES["choosefile"]["tmp_name"]) && $_FILES["choosefile"]["tmp_name"] != "") {
			$sql = insertQ("UPDATE `sptestimonial` SET name=?, designation=?, image=?, description=? WHERE id=?", "ssssi", [$name, $designation, $urll,$txtDesc, $hidId]);
        }else{
			$sql = insertQ("UPDATE `sptestimonial` SET name=?, designation=?, description=? WHERE id=?", "sssi", [$name, $designation,$txtDesc, $hidId]);
		}
    ...
    function deletee($dbConn) {
		...
		$sql = insertQ("DELETE FROM sptestimonial  WHERE id=?", "i", [$conId]);
		$_SESSION['count'] = 0;
		$_SESSION['data'] = "success";
