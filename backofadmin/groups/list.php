<?php
	...
	$sql = "SELECT * FROM spuser AS u INNER JOIN spprofiles AS p ON u.idspUser = p.spUser_idspUser INNER JOIN spprofiles_has_spgroup AS d ON p.idspProfiles = d.spProfiles_idspProfiles INNER JOIN spgroup AS g ON d.spGroup_idspGroup = g.idspGroup WHERE d.spProfileIsAdmin = 0";
								...
								while ($row = dbFetchAssoc($result)) {
									...
									<tr>
										...
										if (isset($spgroupimage) && $spgroupimage != '') {
										 	echo "<img class='' src='	https://dev.thesharepage.com/uploadimage/" . ($spgroupimage) . "' style='width: 100px;' >";
										} 
										else {
												echo "<img class='' src='https://t3.ftcdn.net/jpg/04/34/72/82/360_F_434728286_OWQQvAFoXZLdGHlObozsolNeuSxhpr84.jpg' style='width: 100px;' >";
										}
										...
										<td class="menu-action text-center">
											...
											?>
											<a href="<?php echo $BaseUrl.'/grouptimelines/?groupid='.$idspGroup.'&groupname='.$spGroupName.'&timeline&page=1';?>" data-original-title="Detail" data-toggle="tooltip" data-placement="top" class="btn menu-icon vd_bg-yellow"> <i class="fa fa-info"></i> </a>
