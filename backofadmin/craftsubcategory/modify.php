<?php
	...
	$sql = "SELECT * FROM craft_subcategory WHERE idspCraftgallery ='$ArtCat'";
				...
				<div class="box-body">
					...
					<div class="row">
						<div class="col-md-6 col-sm-6" style="margin-bottom:20px;">
							...
							<?php 
								$sql1 =  "SELECT * FROM craft_category ";
							...
							if ($result1) {
								...
								while ($row1 = dbFetchAssoc($result1)) {
									//extract($row1);
							?>
								<option <?php if($row1['id']==$idspCraftcategory){ echo 'selected'; } ?> value="<?php echo $row1['id'];?>"><?php echo $row1['craft_title']; ?></option>
							<?php }
									}
							?>
							</select>
						...
						<div class="col-md-6 col-sm-6" style="margin-bottom:20px;">
							<label>Name</label></br>
							<input type="text" name="txtTitle" id="txtTitle" value="<?php echo $spCraftgalleryTitle;?>" class="form-control" required="required"/>
						</div>
