<?php
	...
	$rowsPerPage = 25;
  	$sql		=	"SELECT * FROM spwalletartandcraft";
	...
	//$pagingLink = getPagingLink($dbConn, $sql, $rowsPerPage);
 	if(isset($_POST['submit'])&& ($_POST['selectname']!=0))
	{
		$sdate=$_POST['sdate'];
  $edate=$_POST['edate'];
 ...
 // echo 1;
   $query2="SELECT * FROM spwalletartandcraft WHERE buyer_userid='$selectname' AND date_txn BETWEEN '$sdate' AND '$edate'";
  }
  else if(($sdate && $edate)&& !($selectname)){
  // echo 2;
  $query2="SELECT * FROM spwalletartandcraft WHERE  date_txn BETWEEN '$sdate' AND '$edate'";
  }
   else if(($selectname)&& !($sdate && $edate)){
   //echo 3;
   $query2="SELECT * FROM spwalletartandcraft WHERE buyer_userid='$selectname' ";
  }
...
?>
	...
	<!-- Content Header (Page header) -->
		...
		<h1> Art and Craft Wallet<small>[List]</small></h1>
		<form action="" method="POST">
		
		Select Name : <select id='selUser' name="selectname"  style="width:450px;"> 
		<option value="0">Select User</option>
		<?php 
		$query="SELECT * FROM spuser";
		$ress=dbQuery($dbConn,$query);
		...
		<a href="<?php  $_SERVER["DOCUMENT_ROOT"] ?>/backofadmin/walletartandcraft/index.php?view=artandcraftlist" class="btn btn-primary">Reset</a>
	</section>
	<!-- Main content -->
...
<a href="<?php  echo WEB_ROOT ?>/backofadmin/registerdUser/index.php?view=detail&uid=<?php echo $row12['idspUser']; ?>" >                      <?php echo $row12['spUserFirstName'].' '.$row12['spUserLastName'];


                            ?></a></td>
						<td><?php echo $row11['amount'];
						$total=$total+intval($row11['amount']);
						
						?></td>
						<td><?php echo $row12['currency']; ?></td>
						<td><?php 
                                $userid2=$row11['seller_userid'];
...
<a href="<?php  echo WEB_ROOT ?>/backofadmin/registerdUser/index.php?view=detail&uid=<?php echo @$row13['idspUser']; ?>" >                      <?php echo @$row13['spUserFirstName'].' '.@$row13['spUserLastName'];
