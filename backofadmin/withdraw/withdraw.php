...
<?php
	...
	function addremark($dbConn){
		...
		$withdraw_id   = mysqli_real_escape_string($dbConn,$_POST['withdraw_id']);
		...
		if(dbNumRows($result2) > 0){
			...
			// Insert
			$sql = "UPDATE spevent_transection SET remark ='$remark' WHERE id = $withdraw_id";
			$result = dbQuery($dbConn, $sql);
			//$result = dbQuery($dbConn, $sql);
			$_SESSION['count'] = 0;
			$_SESSION['errorMessage'] = "Remark Added Successfully.";
			$_SESSION['data'] = "success";
			redirect('index.php?view=event');	
		/*}*/
	}
...
$update_sql = "UPDATE withdraw_amount SET withdrawstatus='$sp_status' WHERE userid= '$sp_id'";
