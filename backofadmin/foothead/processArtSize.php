...
<?php
	...
	function modify($dbConn){
		...
		$sql2 = "SELECT fh_id FROM tbl_foot_heading WHERE fh_title = '$txtTitle'";
		...
		if(dbNumRows($result2) > 0){
			...
			// Insert
			$sql = "UPDATE tbl_foot_heading SET fh_title = '$txtTitle' WHERE fh_id = $hidId";
	...
	function add($dbConn){
		...
		if(dbNumRows($result2) > 0){
			...
			// Insert
			$sql   = "INSERT INTO tbl_foot_heading (fh_title) VALUES ('$txtTitle')";
			$result = dbQuery($dbConn, $sql);
			$_SESSION['count'] = 0;
	...
	function deletee($dbConn){
		...
		$sql		=	"DELETE FROM tbl_foot_heading WHERE fh_id = $id";
		$result 	= 	dbQuery($dbConn, $sql);
		$_SESSION['count'] = 0;
