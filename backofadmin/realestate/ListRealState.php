<?php
	...
	if(isset($_GET['pid'])){
		$pid = $_GET['pid'];
		$sql="SELECT * FROM sprealstate WHERE spProfiles_idspProfiles = $pid ORDER BY idspPostings ASC";
		}else{
  	$sql="SELECT * FROM sprealstate  ORDER BY idspPostings DESC";
		}
							...
							if ($result){
								...
								while($row = dbFetchAssoc($result)) {
									...
									<tr>
										...
										<td><?php echo $spPostingPrice . ' ' . $defaltcurrency; ?></td>
										<td><?php echo $spPostListing; ?></td>																		   										<td class="text-center"><?php echo $status; ?></td>

										<td class="text-center menu-action">						
											<a href="<?php echo $BaseUrl; ?>/real-estate/property-detail.php?postid=<?php echo $idspPostings; ?>" data-toggle="tooltip" title="Info" class="btn menu-icon vd_bg-yellow"><i class="fa fa-info"></i></a>&nbsp; 										
										</td>
									</tr><?php
									$i++;
								}
