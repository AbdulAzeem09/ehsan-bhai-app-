<?php
	...
	$rowsPerPage = 25;
  	$sql		=	"SELECT * FROM sprealstate_wallet";
...
//echo $_POST['selectname'];
  ...
  if($sdate && $edate && $selectname){
   $query2="SELECT * FROM sprealstate_wallet WHERE seller_userid='$selectname' AND date_txn BETWEEN '$sdate' AND '$edate'";
  }
  else if(($sdate && $edate)&& !($selectname)){
   $query2="SELECT * FROM sprealstate_wallet WHERE  date_txn BETWEEN '$sdate' AND '$edate'";
  }
  else if(($selectname)&& !($sdate && $edate)){
   $query2="SELECT * FROM sprealstate_wallet WHERE seller_userid='$selectname'";
  } 
...
?>
	...
	<!-- Content Header (Page header) -->
		...
		<h1> Real Estate Wallet<small>[List]</small></h1>
		...
		<input type="submit" name="submit" value="Submit" id="submit" class="btn btn-primary">
			...
			<a href="<?php  $_SERVER["DOCUMENT_ROOT"] ?>/backofadmin/walletrealestate/index.php?view=realestatelist" class="btn btn-primary">Reset</a>
	</section>
	<!-- Main content -->
              	...
              	<table id="example1" class="table table-bordered table-striped tbl-respon2">
	                <thead>
	                 	<tr>
							...
							<th class="text-center">Balance Transaction</th>            
	                ...
	                <tbody>
					 ...
					 while($row11=mysqli_fetch_assoc($result))
						...
						<td class="text-center"><?php echo $row11['id'];?></td>
						<td class="text-center"><?php  $userid=$row11['buyer_userid'];
 ...
 <a href="<?php  $_SERVER["DOCUMENT_ROOT"] ?>/backofadmin/registerdUser/index.php?view=detail&uid=<?php echo $row12['idspUser']; ?>" ><?php    echo $row12['spUserFirstName'].' '.$row12['spUserLastName'];
                            ...
                            ?></a></td>
						<td class="text-center"><?php echo $row11['amount'];
						$total=$total+$row11['amount'];
						
						?></td>
							<td class="text-center"><?php echo $row12['currency']; ?></td>
 ...
 <a href="<?php  $_SERVER["DOCUMENT_ROOT"] ?>/backofadmin/registerdUser/index.php?view=detail&uid=<?php echo $row13['idspUser']; ?>" ><?php echo  $row13['spUserFirstName'].' '. $row13['spUserLastName'];
                                   ...
                                   ?></a></td>
						<td class="text-center"><?php echo $row11['balanceTransaction'];?></td>
						<td class="text-center"><?php echo $row11['date_txn'];?></td>
