<?php
  ...
  if($sdate && $edate && $selectname){
   $query2="SELECT * FROM speventwallet WHERE buyer_userid='$selectname' AND date_txn BETWEEN '$sdate' AND '$edate'";
  }
 else if(($sdate && $edate)&& !($selectname)){
  $query2="SELECT * FROM speventwallet WHERE  date_txn BETWEEN '$sdate' AND '$edate'";
  }
   else if(($selectname)&& !($sdate && $edate)){
  $query2="SELECT * FROM speventwallet WHERE buyer_userid='$selectname' ";
  }
...
?>
	...
	<!-- Content Header (Page header) -->
		...
		<h1> Event Wallet<small>[List]</small></h1>
		...
		<a href="<?php  $_SERVER["DOCUMENT_ROOT"] ?>/backofadmin/walletevent/index.php?view=eventlist" class="btn btn-primary">Reset</a>
	</section>
	<!-- Main content -->
