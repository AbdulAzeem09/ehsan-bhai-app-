...
// define('BASEURL', 'https://thesharepage.com/');

// define ('DBPATH','localhost');
// define ('DBUSER','theshare_page');
// define ('DBPASS','mgB81jXxxafr');
// define ('DBNAME','theshare_share');
// define('BASEURL', 'https://localhost/');
include($_SERVER['DOCUMENT_ROOT'].'/univ/main.php');
include($_SERVER['DOCUMENT_ROOT'].'/univ/baseurl.php');  

define('BASEURL', $BaseUrl);  
//require_once($BaseUrl . "/univ/main.php");
session_start();
...
if ($_GET['action'] == "getImageOfMyChat"){ getImageOfMyChat($dbh); }
	...
	function getImageOfMyChat($dbh){
		...
		if($result){
			$row = mysqli_fetch_assoc($result);
			if ($row['spProfilePic'] != '') {
				 echo '<img src="'.($row['spProfilePic']).' " class="img-responsive">';
			}else{
				echo "<img = src='".BASEURL."assets/images/blank-img/default-profile.png' alt='' class='img-responsive' />";
			}
...
function chatHeartbeat($dbh) {
	...
	while ($chat = mysqli_fetch_array($query)) {
		...
		$items .= <<<EOD
			...
			"k": "{$chat['spprofiles_idspProfilesSender']}",
...
EOD;
	...
	foreach ($_SESSION['openChatBoxes'] as $chatbox => $time) {
		if (!isset($_SESSION['tsChatBoxes'][$chatbox])) {
			...
			//ashish here client area time zone declare.
...
EOD;
	...
	$sql = "INSERT INTO spfriendchatting(spfriendChattingMessage, spfriendchatting.spprofiles_idspProfilesSender, spfriendchatting.spprofiles_idspProfilesReciver) VALUES ('".$message."', '".$from."', '".$to."')";
	...
	//echo "1";
	// ===SHOW USER IMAGE
	$sql2 = "SELECT * FROM spprofiles WHERE idspProfiles = $from";
