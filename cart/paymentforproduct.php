...
if(!isset($_SESSION['pid'])){
  ...
  if(!$cardExpMonth){
	  die("cardExpMonth invalid");
	}
  $cardExpYear = !empty($_POST['cardExpYear']) ? $_POST['cardExpYear'] : "";
  if(!$cardExpYear){
	  die("cardExpYear invalid");
	}
  $cardString = strtolower($customerName)."||".$cardNumber."||".$cardExpMonth."||".$cardExpYear."||".$cardCVC;
	$currency = !empty($_POST['currency_code']) ? $_POST['currency_code'] : "";
  //$currency = "USD"; //testing
  if(!$currency){
	  die("currency invalid");
	}
  $orderQty = !empty($_POST['spOrderQty']) ? $_POST['spOrderQty'] : "";
  if(!$orderQty){
	  die("orderQty invalid");
	}	
	/**/
  ...
  //$cardNumber = "21212"; //testing
  /**CARD storage**/
	$carddetails = array(
	  $customerName,
		encryptMessage($cardNumber),
		$cardExpMonth,
		$cardExpYear,
		$cardCVC,
		$_SESSION['uid']
	);
	insertQ("update spuser set customerName=?, cardNumber=?, cardExpMonth=?, cardExpYear=?, cardCVC=? WHERE idspUser=?", "ssiiii", $carddetails);
  /**/
  ...
  //echo "<pre>"; print_r($_POST);die;
	...
	if($stripeToken){
		...
		$it = $pi->readtypeitembystore($_SESSION['uid'], $seller_id); //store
    ...
    $famount=0;
      ...
      die("Cart is empty");
		...
		$it11 = $pi->readtypeitembyartandcraft($_SESSION['uid'], $seller_id); //artandcraft
    ...
    if(($famount+$famt) != $total_amount){
      die("Pay amount and cart-amount mismatches");
    }
    ...
    //echo '<pre>';var_dump($_POST['cardDetails']);die;
...
try{
  ...
  $paymenyResponse = $payDetails->jsonSerialize();
	...
	$paymentMessage = $_SESSION['pfailed_reason'] = 'Your card was declined '. $e->getMessage().'card_declined '.$e->getStripeCode().'generic_decline '.$e->getDeclineCode().'exp_month '. $e->getStripeParam();
}
catch (Error\InvalidRequest $e) {
	$paymentMessage = $_SESSION['pfailed_reason'] = "<strong>".ucfirst($e->getStripeParam())."</strong> ".$e->getMessage();
}
catch (\Exception $e) {
	$paymentMessage = $_SESSION['pfailed_reason'] = "<strong>".ucfirst($e->getStripeParam())." </strong> ".$e->getMessage();
}

//var_dump($paymentMessage);die;

$orderid = "";
$p = new _order;
$rpvtforss = $p->readselleritem_sold($_SESSION['uid'], $seller_id);
if($rpvtforss!=false){
...
if($paymenyResponse['amount_refunded'] == 0 && empty($paymenyResponse['failure_code']) && $paymenyResponse['paid'] == 1 && $paymenyResponse['captured'] == 1){
	...
	$ba=$pi->readfrombasket($_SESSION['uid'], $seller_id);
...
//print_r($ba1);die('===444');
			...
			if($ba1['cartItemType']=='Store'){
				...
				if($spr!=false){
					...
					$product_title=$spr1['spPostingTitle'];
			...
			if($ba1['cartItemType']=='artandcraft'){
				...
				if($art!=false){
					...
					$product_title=$art1['spPostingTitle'];
...
// $cardDetails
	...
	$seller_id1 = $p->spproid($seller_id);
	$rowss=mysqli_fetch_assoc($seller_id1);

	$spname=$rowss['spProfileName'];
	$spemail=$rowss['spProfileEmail'];
	$postid=$rowss['idspPostings'];
	$total_pricemail= $paidCurrency.' '.$total_amount;

	$seller_id2 = $p->spproid($_SESSION['pid']);
	$rows1=mysqli_fetch_assoc($seller_id2);
	$bookeduseremail =   $rows1['spProfileEmail'];

// echo $bookeduseremail;
//  die('=========================');
	$e = new _email; 
// email to event Organizer
	$e->sendproductsold($spemail, $spname, $product_title,$orderQty,$total_pricemail,$postid);

	$e->sendproductsold_customer($bookeduseremail,$customerFname,$product_title,$orderQty,$total_pricemail,$postid);

	$row34 = selectQ("SELECT * FROM addshipping_address WHERE uid=? AND status=1", "i", [$_SESSION["uid"]], "one");
...
$spprofiole= $sppr->read($seller_id);
...
$spUser= $u->read($spUser_idspUser);
$getSpUser = mysqli_fetch_assoc($spUser);

$tier2_user = $mb->getReferrelUserByCode($getSpUser['refferalcodeused']);
$tier2_userid = mysqli_fetch_assoc($tier2_user);
$tier3_user = $mb->getReferrelUserByCode($tier2_userid['refferalcodeused']);
...
$data = array(
	...
	"currency"=>$currency,
	"date"=>$date,
	'tier1_userid'=>$tier2_userid['idspUser'],
	'tier2_userid'=>$tier3_userid['idspUser'],
	'tier1_commission'=>$tier2_commission,
	'tier2_commission'=>$tier3_commission,
