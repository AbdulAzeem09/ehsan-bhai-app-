   ...
   .ratings:before {
	...
	<body onload="pageOnload('cart')" class="bg_gray">
            		...
            		<div class="col-md-9">
                    	<div class="cartbox bradius-15" style="margin-top: 10px;">
                    		<div class="cart_header">
                    			<h3 style="color: #032350;"><i class="fa fa-shopping-cart"></i>&nbsp;Added to Cart</h3>
                    		</div>
                    		<div class="cart_body">
                    			<?php
									$p = new _order;
									...
									$totalpricenewnewforss=0;
										while($rowrpvtforss = mysqli_fetch_assoc($rpvtforss))
											{ 
												$totalpricenewnewforss +=	$rowrpvtforss['sporderAmount'];
											}
		...
		<div class="row">
			...
			<p style="float:right;">
				<a href="#" data-toggle="modal" data-target="#exampleModal" onclick="payOnlyThisSeller('<?php echo $values; ?>','<?php echo $totalpricenewnewforss; ?>');">Pay Only This Seller</a>	|
		...
		<hr style="border: -1px solid Grey;">
						  ...
						  <?php $rpvt = $p->readselleritem($_SESSION['uid'],$values);
									...
									if ($rpvt != false){
										...
										<?php		
												...
												/*if ($res != false){
													...
													$postingid = $row["spPostings_idspPostings"]; 
													$totalqtyprice = $spOrderQtyghg * $row['spPostingPrice'];
													$price = $row['spPostingPrice'];
													$totalprice+=$totalqtyprice;
													
														echo"<div class='row' style='padding-left:190px'>
																<div class='col-md-3' style='width:180px;margin-left:-176px;float:left;border: 1px solid gray;border-radius: 11px;padding-top: 10px;
																padding-bottom: 10px;'>";
																if ($itemType =='Training') {
																		echo "<a href='../trainings/detail.php?postid=".$row["spPostings_idspPostings"]."'> ";
																			$pc = new _postingpic;
																			$tr = new _postingview;
																			...
																			if ($res != false) {
				   															...
				   															<img src="<?php echo ($picture); ?>" alt="Posting Pic" class="img-responsive"  style='height: 100px; width: 130px;'>

																		<?php 
																			$active2++;
																			}else{
																			echo"<img src='../img/no.png' alt='Posting Pic' class='img-responsive' style='margin: 0 auto;'' style='height: 100px; width:130px;'>";
																			}
																...
																<?php 
																	...
																	echo"</div>
																	<div class='col-md-11' style='padding-left:0%;float:left;padding-right: 0px;'>
																	...
																	if ($itemType == 'Training') {
																	 ...
																	 style='font-size: 24px;'>".ucfirst($row['spPostingTitle'])."</a>";
																		...
																		echo "<a class='a-link-normal sc-product-link' href='../artandcraft/detail.php?postid=".$row["idspPostings"]."'
																	 style='font-size: 24px;'>".ucfirst($row['spPostingTitle'])."</a>";
																	}
                                   ...
                                   if($itemType != 'Training'){
                                      ...
                                      <?php 
				                               ...
				                               echo" <div class='col-md-3' style='float: right; padding-bottom: 25px; padding-left: 0px; color:red;'>
				                                        <h4><b style='font-size: 20px;margin-right: 4px;'> $".$mainpricenow."</b></h4> </div>";
                                    	}
																	}else{
																		$tr_quantity=1;
																		$tr_order = new _order;
                ...
                echo"<div class='col-md-12 no-padding' style=' float: right; margin-bottom: 25px; '>

									<h4><b style='float: right; font-size: 22px; padding-left: 28px; padding-right: 22px; color:#202548;'>Cart Subtotal (".$cartitem." item): $<sapn id='totalamount'>".$totalpricenewnew."</span></b></h4>
...
<!---<hr  style='border: 1px solid Grey;'>--->
<a href="../my-profile/add-shipping.php" style="float: right;margin-bottom: 26px;">Change Shipping Address</a>
<h3 style="color: #032350;">Shipping Address</h3>
...
<!-- saved Product Start -->
								<div class="cartbox bradius-15" style="margin-top: 10px;">
									...
									<div class="cart_body">
										<?php
											$p = new _order;
											$rpvt = $p->readCartItemsaved($_SESSION['uid']);
											if ($rpvt != false){
											  	$cartitem=0;
											  	while($row = mysqli_fetch_assoc($rpvt))
													{ 	
												//print_r($row ); die('-------------------');
														...
														$title = $row['spPostingTitle'];
														$m = new _postfield;
														$res = $m->readfield($row["spPostings_idspPostings"]);
														if ($res != false){
															while($rows = mysqli_fetch_assoc($res))
															{ 
																if($rows["spPostFieldLabel"] == "Start Date"){
													...
													//Quantity Availability
													$pr = new _postfield;
													$re = $pr->quantity($row["spPostings_idspPostings"]);
													if($re != false)
													{
														$i = 0;
														$rw = mysqli_fetch_assoc($re);
													...
													$or = new _order;
													$soldquantity  = 0;
													$res = $or->quantityavailable($row["spPostings_idspPostings"]);
													if($res != false)
													{
														while($order = mysqli_fetch_assoc($res))
														{
												...
												//Quantity Availability Completed
													...
													$totalprice+=$totalqtyprice;
													echo"<div class='row' style='padding-left:190px'>
													<div class='col-md-3' style='width:190px;margin-left:-190px;float:left;'>";
													...
													<ul class='a-unordered-list a-nostyle a-vertical a-spacing-mini'>
														<li>";
															if ($SaveditemType == 'Training') {
																		echo "<a class='a-link-normal sc-product-link' href='../trainings/detail.php?postid=".$row["spPostings_idspPostings"]."'
																	 style='font-size: 24px;'>".$title."</a>";
																	}else{
																		
									    $rpvtnewad = $p->resdnewagainnewsa($row["spPostings_idspPostings"]);
										
											$rownewadss = mysqli_fetch_assoc($rpvtnewad);
											
																		echo "<a class='a-link-normal sc-product-link' href='../store/detail.php?postid=".$row["spPostings_idspPostings"]."'
                            		...
                            		echo"<div class='col-md-3' style='float: right; padding-bottom: 25px; padding-left: 0px; color:red;'>
                                			<h4><b style='font-size: 20px;'> $".$rownewadss['spPostingPrice']."</b></h4> 
																		...
																		$p = new _postingview; //For price and discount price
																		$pf  = new _postfield;
																		$result_pr = $p->singletimelines($row['spPostings_idspPostings']); //For price and discount price
																		$row_pr = mysqli_fetch_assoc($result_pr); //For price and discount price
																		$result_pf = $pf->read($row['spPostings_idspPostings']);//For price and discount price
																		$txtDiscount = "";
																			 while ($row2 = mysqli_fetch_assoc($result_pf)) {
																				 	if($txtDiscount == ''){
										                        if($row2['spPostFieldName'] == 'txtDiscount_'){
	                    									...
	                    									$exactPrice = ceil($discountedPrice);
																		...
																		$tr_res = $tr_order->readIdOrder($_SESSION['pid'],$row['spPostings_idspPostings']);
																		$tr_order = mysqli_fetch_assoc($tr_res);
...
if(!empty($_POST['stripeToken'])){
   ...
   $e->sendeventbooked($postedusername,"ecommerceguru13@gmail.com",$event_title,$bokkedbynameu,$orderQty,$totalAmount);
		...
		$tr_id = $pet->createagain($data); 
									...
									$paass->updateorderstatusnew($_SESSION['pid'], $seller_id);
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                ...
                <div class="left_grid left_group_black left_sidebar" >
                	 ...
                	 <span style="font-weight: bold;   font-size: large; text-align: center;" >Recently Viewed Products </span>
...
//print_r($pic);
           ...
           // $resv = $pv->readrecentview($pro_id,$_SESSION['uid']);
            $resv = $pv->readrecentcartview($_SESSION['uid']);

           // echo $resv->ta->sql;
            if ($resv != false) {
                    
                    //$rowf = mysqli_fetch_assoc($resv);
                    //print_r($rowf['productid']);

                    while($rowf = mysqli_fetch_assoc($resv)){
                    	//print_r($rowf);

                  /*  	if ($rowf['productid'] == $pro_id) { */
...
//print_r($pic);
                     ...
                     <div class="row" style="margin-bottom: 20px;margin-top: 15px;">
                       <div class="col-md-12">
                    		<div class="col-md-4">
                    			 <a  href='<?php echo $BaseUrl.'/store/detail.php?catid=1&postid='.$rowf['productid'];?>'> 


   <img src="<?php echo ($picture); ?>" alt="Posting Pic" class="img-responsive"  style='height: 85px; width: 100px; padding-bottom: 10px;'>

<?php 
$active2++;
/*}*/
}else{?>
<img src='../img/no.png' alt='Posting Pic' class='img-responsive' style='margin: 0 auto;height: 85px; width: 100px; padding-bottom: 10px;'>
<?php }?>
</a>


                    			<!--   <img src="<?php echo $BaseUrl.'/assets/images/logo/tsplogo.PNG' ?>" alt='logo' style='height: 100px; width: 100px;' class='sc-product-image' /> -->
                        </div>
                    
                    <div class="col-md-8">
                    <ul class='a-unordered-list a-nostyle a-vertical a-spacing-mini' style="margin-bottom: 5px;">
						<li  style="padding-left: 0px;">
						<a class='a-link-normal sc-product-link' 
						 style='font-size: 15px; color: #202548; text-transform: capitalize;'><?php echo $prodetail['spPostingTitle'];?></a> 
						</li>
                    </ul>
                      <!-- <span class="margin-top-6">
                       ...
                       <p id='eventrating' class="rating"style="    margin-bottom: 5px; line-height: 6px; ">
 ...
 -->
                         <input class="stars" type="radio" id="star5" name="rating" value="5" <?php if($raterow['rating'] == "5") { echo "checked "; }?> 
                              />
                                            <label  style="cursor:pointer ;color: #FDCC0D; <?php if($raterow['rating'] >= "5") { echo "color: gold";}?>" class = "full" for="star5" title="Awesome - 5 stars" style="color: gold!important;"></label>

                                            <input class="stars" type="radio" id="star4" name="rating" value="4" <?php if($raterow['rating'] == "4") { echo "checked";}?> />

                                            <label style="cursor:pointer; color: #FDCC0D;<?php if($raterow['rating'] >= "4") { echo "color: gold";}?>" class = "full <?php if($raterow['rating'] == "4") { echo "checkrating "; }?>" for="star4" title="Pretty good - 4 stars"></label>

                                            <input class="stars" type="radio" id="star3" name="rating" value="3" <?php if($raterow['rating'] == "3") { echo "checked";}?> />


                                            <label style="cursor:pointer; color: #FDCC0D;<?php if($raterow['rating'] >= "3") { echo "color: gold";}?>" class = "full" for="star3" title="Meh - 3 stars"></label>

                                            <input style="cursor:pointer;" class="stars" type="radio" id="star2" name="rating" value="2" <?php if($raterow['rating'] == "2") { echo "checked";}?> />

                                            <label style="cursor:pointer; color: #FDCC0D; <?php if($raterow['rating'] >= "2") { echo "color: gold";}?>" class = "full" for="star2" title="Kinda bad - 2 stars"></label>

                                            <input class="stars" type="radio" id="star1" name="rating" value="1" <?php if($raterow['rating'] == "1") { echo "checked";}?> />

                                            <label style="cursor:pointer;color: #FDCC0D; <?php if($raterow['rating'] >= "1") { echo "color: gold";}?>" class = "full" for="star1" title="Sucks big time - 1 star"></label>

                                           <!--  <span><?php if($totalrate <= 0 ){ echo "0.0"; }else{ echo $totalrate; } ?></span> -->
                                </p>
...
<div class="a-row" style="margin-bottom: 10px;">
	...
	<span class="a-size-base a-color-price">
		<span class="p13n-sc-price">$&nbsp;<?php echo $prodetail['spPostingPrice'];?></span></span>
    </a> 

</div>


 <a class="btn-border-radius" href="<?php echo $BaseUrl.'/store/detail.php?catid=1&postid='.$rowf['productid'];?>" style="background-image: -webkit-linear-gradient(90deg,#202548 0,#202548 100%)!important;border-color: #202548!important;color: white;padding: 6px 11px 6px 11px;">View</a>
<!-- <button type="button" style="  background-image: -webkit-linear-gradient(90deg,#202548 0,#202548 100%)!important;
...
<!-- <div class="row">
<span class="glyphicon glyphicon-star-empty"><span class="glyphicon glyphicon-star-empty"><span class="glyphicon glyphicon-star-empty"><span class="glyphicon glyphicon-star-empty"><span class="glyphicon glyphicon-star-empty"></span></div> -->

<!-- <div class="a-row">
	<a class="a-link-normal a-text-normal">
	<span class="a-size-base a-color-price">
		<span class="p13n-sc-price">₹&nbsp;179.00</span></span>
  </a> 
</div> -->
