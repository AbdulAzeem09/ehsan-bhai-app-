...
<?php
$p = new _postings;
//$result = $p->read($postId);
if($postId) {
$row = selectQ("select * from sppostings AS t left join spgroup as spg on t.groupid = spg.idspgroup where t.idsppostings =? and t.sppostingsstatus =?", "ii", [$postId, 0], "one");
...
/*print_r($row);*/
echo "<input type='hidden' id='postititle' value='" . $spPostingtitle . "' name='posttitle'>";

//echo "<input type='hidden' id='shareprofile' value='".$row["spProfileName"]."' name='profilename'>";

echo "<input type='hidden' id='sharedpost' value='" . $idspPostings . "' name='postid'>";
}
$p2 = new _postings;
...
//print_r($row);
//die("mknj");
$r_id = $row["idspProfiles"];
$href_profile22 = $BaseUrl . "/friends/?profileid=" . $r_id;
...
//$result11 = $timeline->singletimelines($postId);
if($postId){
  $result11 = selectQ("select * from sppostings AS t left join spgroup as spg on t.groupid = spg.idspgroup where t.idsppostings =? and t.sppostingsstatus =?", "ii", [$postId, '']);
}

if (isset($result11) && count($result11) > 0) {
...
<a href='<?php echo $BaseUrl . "/friends/?profileid=" . $r_id; ?>'><h4><?php echo $name; ?></h4></a>
...
<h5>
<!-- <?php echo $postingDate; ?> -->1 day ago <i class="fa fa-globe"></i>
</h5>
</div>
</div>
<div class="col-md-1">
...
/* print_r($_SESSION['pid']);https://dev.thesharepage.com/post-ad/savePost.php?save=2322
print_r($row);myFunction()*/

if ($_SESSION['pid'] == $row['spProfile_idspProfile']) { ?>

<div class="dropdown pull-right right_profile_timeline">
<button class="btn dropdown-toggle" onclick="" type="button" data-toggle="dropdown"><i class="fa fa-ellipsis-h" aria-hidden="true"></i></button>
<ul class="dropdown-menu" id="myDropdown">
<?php
if (isset($_SESSION['pid'])) {
$sp = new _savepost;

$result2 = $sp->savepost($postId, $_SESSION['pid'], $_SESSION['uid']);
if ($result2) {
if ($result2->num_rows > 0) { ?>
<li><a onclick="unsave_post('<?php echo $BaseUrl . '/post-ad/savePost_comment.php?unsave=' . $rows['idspPostings']; ?>')"><i class="fa fa-save"></i> Unsave Post</a></li> <?php
} else { ?>
<li><a onclick="save_post('<?php echo $BaseUrl . '/post-ad/savePost_comment.php?save=' . $rows['idspPostings']; ?>')"><i class="fa fa-save"></i> Save Post</a></li> <?php
}
} else { ?>
<li><a onclick="save_post('<?php echo $BaseUrl . '/post-ad/savePost_comment.php?save=' . $rows['idspPostings']; ?>')"><i class="fa fa-save"></i> Save Post</a></li> <?php
}
} else { ?>
<li><a href="<?php echo '=' . $rows['idspPostings']; ?>"><i class="fa fa-floppy-o"></i> Save Post</a></li> <?php
}
...
if ($rows["spPostingNotes"] != '') {
  echo "<div style='color:#333;padding-top:20px;word-wrap: break-word; width: 480px'>" . html_entity_decode($rows["spPostingNotes"]) . "</div>";
}
...
$reply_data = $comment->reply_res($rows['idspPostings']);
$replytotalcmt = $reply_data->num_rows;
?>
<?php 
...
$result_cs = $comment->read($postId);
$rowsc = 0;
if($result_cs){
$rowsc =mysqli_num_rows($result_cs);
}
?>

<li><span data-toggle='tooltip' data-placement='bottom' title='Comment' style="margin: 10;"><i class="fa fa-comment" aria-hidden="true" style="font-size:21px;"></i> <span class='font_regular'></span></span> <span>(<?php echo $rowsc + $replytotalcmt; ?>)</span></li>
<li>
<?php
$pl = new _favorites;
...
echo "<span id='spFavouritePost' data-toggle='tooltip' data-placement='bottom' title='Unfavourite' class='icon-favorites fa fa-heart removefavorites faa-pulse animated1 33' style='font-size:21px'  data-postid='" . $rows['idspPostings'] . "'><span class='font_regular'></span></span>
<span onclick=\"postfunction(" . $rows['idspPostings'] . ")\" class='show-modal' id='delid".$rows['idspPostings']."' style=''>(".$count.")</span>";
$i++;  
} else {
//Favourite

echo "<span id='spFavouritePost'  data-placement='bottom' title='Favourite' style='
font-size: 20px;' class='icon-favorites fa fa-heart-o sp-favorites faa-pulse animated1 22' data-postid='" . $rows['idspPostings'] . "'><span id='text_" . $rows['idspPostings'] . "'>  </span><span class='font_regular'></span></span>";

}
}
// if ($i == 0) {

// echo "<span id='spFavouritePost' data-toggle='tooltip' data-placement='bottom' title='Favourite' class='icon-favorites fa fa-heart-o sp-favorites faa-pulse animated1' style='font-size:21px' data-postid='" . $rows['idspPostings'] . "'><span class='font_regular'> </span></span>";
// }
} else {

echo "<span id='spFavouritePost' data-toggle='tooltip' data-placement='bottom' title='Favourite' class='icon-favorites fa fa-heart-o sp-favorites faa-pulse animated1 11' style='font-size:21px' data-postid='" . $rows['idspPostings'] . "'><span class='font_regular'></span></span>
<span onclick=\"postfunction(" . $rows['idspPostings'] . ")\" class='show-modal' id='delid".$rows['idspPostings']."' style=''>(".$count.")</span>";
} ?>
</li>
...
?>
<li><span data-toggle='tooltip' data-placement='bottom' title='Share'><a href="javascript:void(0);" data-toggle='modal' data-target='#myshare'><span class='sp-share' data-postid='<?php echo $rows['idspPostings']; ?>' src='<?php echo ($pict); ?>'><i class="fa fa-share-alt" style="font-size:21px;"></i><span class='font_regular'></span></span></a></span><span class="show-modal-share" onclick="openshare(<?php echo $rows['idspPostings'];?>)" style='font-size:14px;margin-left: 5px;'><?php echo "($sharecount)"; ?></span> </li>
</ul>
...
//$result_c = $comment->read($postId);
/*                            $result = $c->read($_POST['idspPostings']);
*/

if($postId){
  $result_c = selectQ("select * from comment AS t inner join spprofiles as d on t.spprofiles_idspprofiles = d.idspprofiles where t.sppostings_idsppostings =?", "i", [$postId]);
}

if (isset($result_c) && count($result_c) > 0) {
  foreach ($result_c as $row) {
      ...
      <div class="modal-dialog">
        ...
        <div class="modal-content  bradius-15">
          <form method="post" action="editcomment.php" id="sp-form-post-edit" class="editPostTimeline" enctype="multipart/form-data">
              ...
              <div class="row">
                <div class="col-md-12">
                  ...
                  <div class="">
                    ...
                    <input name="postid" id="pid" type="hidden" value="<?php echo $postId ?>">
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="modal-footer">
              <button id="comment" onclick='editcomment(<?php echo $row["idComment"]; ?>)' class="btn btnPosting pull-right editing db_btn db_primarybtn">Save</button>
    ...
    <input name="spPostings_idspPostings" id="spPostings_idspPostings" type="hidden" value="<?php echo $postId ?>">
    ...
    <div>
    <div id="comment_field" class="cmtdiv<?php echo $cid;  ?>" style="padding: 0px 15px 0px 0px;">
    <div class="col-md-12">
    <div class="col-md-8">
    <?php
    if (isset($picture))
    ...
    <div style="display: inline-flex;padding-left:55px; margin-top:7px; margin-bottom:10px">
      ...
      <a href="javascript:void(0);" class="<?php echo $likeClass; ?>" id="cmnt_like_<?php echo $cid; ?>" data-postid="<?php echo $postId; ?>" data-commentId="<?php echo $cid; ?>" data-userId="<?php echo $_SESSION['pid']; ?>" data-postAction="<?php echo $postAction; ?>">
    ...
    if ($result_rc != false) {
      ...
      ?>
        ...
        if ($cid1 == $cid) {
          ...
          <div class="modal-footer">
          <button id="comment" onclick='editcomment(<?php echo $repid1; ?>)' class="btn btnPosting pull-right editing db_btn db_primarybtn">Save</button>
          <button type="button" class="btn btnPosting pull-right db_btn db_orangebtn" data-dismiss="modal" style="margin-right: 5px;">Cancel</button>
...
success: function(response) {
window.location.href = '<?php echo $BaseUrl; ?>/publicpost/post_comment_details.php?postid=<?php echo $postId; ?> ';
