...
//echo 1;
       // $res2 = $p2->allgrouptimelines($_GET["timelineid"]);
        $res2 = $p2->allgroup_by_shared_Post($_GET["groupid"]);  
...
$("#timediff").val(timezone_offset_minutes);
...
<input type="hidden" name="timediff" id="timediff"  2222222 >  
    <?php

/*   $timezone_offset_minutes =   "<script type='text/javascript'> document.getElementById('timediff').value ;</script>";  //
...
//echo 11;
   $grouptimeline=$grouptimeline+1;
            $postingDate = $p2->spPostingDate($timeline['created']); ?>
                           ...
                           /* print_r($result3);*/
                            if($result3 != false){ 
                                ...
                                <div class="left_profile_timeline">
                                    ...
                                    $PostProfile = $BaseUrl."/friends/?profileid=".$row3['idspProfiles'];
                                    ?>
                                </div>
                                <div class="title_profile">
                                    <h4><?php
                                        echo "<a href='".ucwords(strtolower($sharedProfile))."'>".ucwords(strtolower($profilename3))."</a><i> shared a</i> <a href='".$PostProfile."'</a><i> post</i>"; ?> </h4>
                            ...
                            $result31 = $prof->readUserId($rows["spProfiles_idspProfiles"]);
                            /*print_r($result31);*/
                            if($result31 != false){
                        	/*echo "here";*/
                        	 $row31 = mysqli_fetch_assoc($result31);
                        	   $picture = $row31["spProfilePic"];
                                    $profilename = $row31["spProfileName"];
...
<?php }else{ ?>
   ...
   <i class="fa fa-flag danger" data-toggle="tooltip" data-placement="top" title="Post Flaged" style="color:red;display:none;" id="unflag<?php echo $rows['idspPostings'];?>"  ></i>
              ...
              <?php }else{ ?>
                 <span data-toggle="tooltip" data-placement="top" title="Flag Post">
...
if($flaged_count > 0){
 echo"($flaged_count)";
    ...
    ?>
                            ...
                            <button class=" btn dropdown-toggle 55555555555555555" type="button" data-toggle="dropdown"><i class="fa fa-ellipsis-h" aria-hidden="true"></i></button>
                    ...
                    <div class="col-md-12 ">
                        <h2 style="word-wrap: break-word;">
                            ...
                            <?php if(!empty($proid) && isset($shareby)){
                        ...
                        <?php if(!empty($proid) && isset($shareby)){  ?>
<!-- <a href="<?php echo $BaseUrl.'/store/detail.php?catid=1&postid='.$proid;?>"  target="_blank">View Product</a> -->
                       ...
                       /* echo $pic->ta->sql;*/
                        ...
                        if ($result != false) {
                            ...
                            if (isset($pict)) {
                                ...
                                echo "<a class='thumbnail mag' data-effect='mfp-newspaper' style='border: 0px;' href='" . ($pict) . "'><img alt='Posting Pic' src='" . ($pict) . "' style='height: 50%;width: 50%;' class='postpic img-thumbnail img-responsive bradius-15'></a>";
							...
							if ($result != false) {
								...
								while ($row = mysqli_fetch_assoc($result)) {
									...
									//$date = $row["commentdate"];
										...
										<div class="col-md-12">
											<?php
												echo "<a class='float-right'  href='../publicpost/post_comment_details.php?postid=".$rows['idspPostings']."' ><span class='morecomment' data-postid='" . $rows['idspPostings'] . "' >View all comments45544545 <span class='tltcmt'>" . $totalcmt . "</span></span></a>";
											?>
										</div>
                    ...
                    <div class="col-md-12">
                        <div class="post_footer">
                           ...
                           $pl = new _postlike;
							...
							<div id="new_data_<?php echo $rows['idspPostings'];  ?>">	<a id="currentreaction_<?php echo $rows['idspPostings'];  ?>" href="javascript:void(0);"  class="reactionbtn" data-postid="<?= $rows['idspPostings']; ?>" data-reaction="7" style="font-size: 25px;margin-top: -5px;" ><?php  echo $rection; ?></a>	</div>
							...
							<?php  } else{?>
										...
										<?php if($r->num_rows=='1'){ ?>
												<div id="new_data_<?php echo $rows['idspPostings'];  ?>">		<a id="currentreaction_<?php echo $rows['idspPostings'];  ?>" href="javascript:void(0);"  class="reactionbtn_remove" data-postid="<?= $rows['idspPostings']; ?>" style="font-size: 25px;margin-top: -5px;" ><?php  echo $rection; ?></a>	</div>
							<?php  } }?> 
								...
								<a href="../publicpost/post_comment_details.php?postid=<?php echo $rows['idspPostings']; ?>&loadcom">
									<i class="fa fa-comment" aria-hidden="true"></i> <span class='font_regular'>Comment</span>
									</a>
									</li>
                                <li>
                                    <?php
                                    ...
                                    if ($re != false) {
                                        ...
                                        while ($rw = mysqli_fetch_assoc($re)) {
                                            if ($rw['spUserid'] == $_SESSION['uid']) {
                                                echo "<span id='spFavouritePost' data-toggle='tooltip' data-placement='bottom' title='Unfavourite' class='icon-favorites fa fa-heart removefavorites faa-pulse animated' data-postid='" . $rows['idspPostings'] . "'><span class='font_regular'> </span></span>";
                                                $i++;
                                            }
                                        }
                                        if ($i == 0) {
                                            echo "<span id='spFavouritePost' data-toggle='tooltip' data-placement='bottom' title='Favourite' class='icon-favorites fa fa-heart-o sp-favorites faa-pulse animated 79' data-postid='" . $rows['idspPostings'] . "'><span class='font_regular'></span></span>";
                                        }
                                    } else {
                                ...
                                <li><a href="javascript:void(0);"  data-toggle='modal' data-target='#myshare'><span class='sp-share' data-toggle='tooltip' data-placement='bottom' title='Share' data-postid='<?php echo $rows['idspPostings'];?>' src='<?php echo ($pict); ?>'><i class="fa fa-share-alt"></i> <span class='font_regular'>Share</span></span></a></li>
                            </ul>
